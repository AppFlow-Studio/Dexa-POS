...
export function useCFDWSClient () {
  ...
  const connect = useCallback((options?: { manual?: boolean }) => {
    ...
    const connectTimeout = setTimeout(() => {
      if (ws.readyState !== WebSocket.OPEN) {
        if (__DEV__)
          console.error('[CFD Client] Connection timeout, force-closing')
        ws.close()
      }
    }, CONNECT_TIMEOUT)
    ...
    ws.onclose = () => {
      ...
      if (__DEV__) {
        console.log(
          `[CFD Client] Reconnecting in ${nextDelay}ms... (Attempt ${
            retryCount.current + 1
          })`
        )
      }
  ...
  const sendLoyaltyJoin = useCallback(() => {
    if (wsRef.current?.readyState !== WebSocket.OPEN) return
    const msg: CFDMessage = { type: 'loyalty_join', timestamp: Date.now() }
    wsRef.current.send(JSON.stringify(msg))
  }, [])
