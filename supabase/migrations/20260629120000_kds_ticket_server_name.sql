-- get_kds_tickets_v2: expose creator/server name for KDS ticket headers.
--
-- Ticket: [POS-KDS] Show who placed the order on KDS tickets.
-- Scope:
--   - Add server_id + server_name to the top-level ticket payload.
--   - Resolve the display gate from kds_displays.show_server_name server-side.
--   - Preserve existing ready_time, is_to_go, void/refund notice, and
--     display-scoped acknowledgement behavior.
--
-- Server identity decision: prefer orders.created_by_staff_id and fall back to
-- orders.assigned_server_id. Online / third-party orders with no staff return
-- NULL so the KDS can continue showing the existing source/platform badge.
...
COMMENT ON FUNCTION public.get_kds_tickets_v2(uuid, text[], uuid) IS
  'Returns active KDS tickets with display-scoped acknowledgement, ready_time, is_to_go, and show_server_name-gated server identity.';
