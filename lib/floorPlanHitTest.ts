 ...
 * (rotation is applied about the table's top-left origin, matching how ReadonlyTable
 * / SkiaTableLayer position tables: translate(x,y) then rotate).
 */
export const hitTestTables = (
  ...
  for (const table of ordered) {
    ...
    // Point relative to the table's top-left origin.
    let localX = worldX - table.x;
    let localY = worldY - table.y;

    const rotation = table.rotation || 0;
    if (rotation !== 0) {
      // Undo the table's rotation (rotate the point by -rotation about origin).
