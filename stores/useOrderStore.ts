 ...
 *
 * Mirrors services/sessionEffects/sendToKitchenEffect.ts (the already-hardened
 * table path): send EVERY batch item that has resolved a db_order_item_id by now
 * — regardless of whether it had one when Send was pressed — and queue any
 * still-missing stragglers to the offline queue, which replays the send once
 * their id lands. This closes the "sent N, only M reached the kitchen" race where
 * an item that only got its db id during the sync wait was left to
 * addItemToBackend's retroactive "last sibling" gate, which can race and silently
 * skip it — leaving the item optimistically `sent` locally but `new` in the DB
 * and therefore absent from the KDS (get_kds_tickets_v2 only returns
...
async function _commitKitchenSendForBatch(
  ...
  // the id resolves — same guarantee the table path relies on.
  const stragglerIds = freshItems
  ...
  // Offline, no client, or order not synced yet: queue the whole resolved batch.
...
const addItemToBackend = async (
  ...
  if (!supabase || !isNetworkOnline) {
    if (__DEV__)
      console.log(
        "[addItemToBackend] OFFLINE MODE - Processing item:",
        item.id,
      );
  ...
  try {
    ...
    if (item.is_open_item) {
      ...
      if (addError) {
        ...
        if (__DEV__)
          console.log(
            "[addItemToBackend] open item sync error → queue + surface:",
            addError,
          );
        useSyncStatusStore
          .getState()
          ...
          if (localQuantity !== syncedQuantity) {
            try {
              ...
              if (
                ...
                if (__DEV__)
                  console.log("[QTY-DIAG-MOD] OPEN-ITEM-RECONCILE QUEUE", {
    ...
    if (isMerge && currentDbOrderItemId) {
      ...
      if (updateError) {
        ...
        if (__DEV__)
          console.log(
            "[addItemToBackend] qty update sync error → queue + surface:",
            updateError,
          );
        useSyncStatusStore
          .getState()
    ...
    if (__DEV__ && (item.baseCashPrice == null || item.baseCardPrice == null)) {
      console.warn(
    ...
    if (addError) {
      ...
      if (__DEV__)
        console.log(
          "[addItemToBackend] add item sync error → queue + surface:",
          addError,
        );
      useSyncStatusStore
        .getState()
    ...
    if (addResult?.order_item_id) {
      ...
      // Reconcile per-item "TO GO": the add RPC does NOT persist is_to_go (the
      // column defaults false), so any item whose LIVE state is to-go must be
      // pushed now that the backend row exists. This covers BOTH:
      //   1. added with TO GO already on in the modifier (first-time add), and
      //   2. toggled TO GO while still syncing (no db_order_item_id yet).
      // Read the fresh item (not the stale add-time snapshot) so a to-go set
      // during the sync window isn't dropped. Toggling OFF needs no action here —
      // the DB row is already false. No-op in the common (not-to-go) case.
      const reconcileItemForToGo = useOrderStore
        .getState()
        .ordersById[resolveOrderKey()]?.items.find((i) => i.id === item.id);
      if (reconcileItemForToGo?.is_to_go) {
        OrderService.toggleToGoOnItems(
          supabase,
          [addResult.order_item_id],
          true,
        ...
        if (localQuantity !== syncedQuantity) {
          ...
          try {
            ...
            if (
              ...
              if (__DEV__)
                console.log("[QTY-DIAG-MOD] KITCHEN-RECONCILE QUEUE", {
                  dbItemId: addResult.order_item_id,
                  localQuantity,
                  errCode: (reconcileResp.error as any)?.code,
                });
    ...
    if (__DEV__)
      console.log(
        "[addItemToBackend] caught backend sync error → queue + surface:",
        error,
      );
    useSyncStatusStore
      .getState()
...
const syncPaymentToBackend = async (
  ...
  if (!order.db_order_id) {
    ...
    if (__DEV__)
      console.log(
        `[syncPaymentToBackend] Payment queued for order ${order.id}`,
      );
        ...
        return {
                      ...
                      if (
                        ...
                        const localPendingItems = existingOrder.items.filter(
                          (item) => {
                            ...
                            const status = _syncStore.itemSyncStatus.get(
                              item.id,
                            );
                            const failedAt = _syncStore.itemFailedAt.get(
                              item.id,
                            );
                            if (
                              status === "failed" &&
            ...
            switch (operation) {
              case "INSERT":
                ...
                const existingOrderKey =
                  state.dbOrderIdIndex[dbOrderId] ?? dbOrderId;
                const existingOrder =
                  state.ordersById[existingOrderKey] ?? null;
                if (existingOrder) {
                  if (__DEV__)
                    console.log(
          ...
          batchUpdateItemKitchenStatus: (itemIds, status) => {
            ...
            if (status === "served") {
              ...
              if (
                ...
                order.items.every(
                  (i) =>
                    i.item_status === "served" || i.item_status === "Served",
                ) &&
          ...
          removeItemFromActiveOrder: (itemId, voidReason) => {
            ...
            if (!itemToHandle?.db_order_item_id) {
              cancelPendingByEntity(`item:${itemId}`).catch((err) => {
                if (__DEV__) {
                  console.warn(
                    "[removeItemFromActiveOrder] cancelPending failed:",
                    err,
                  );
                }
              });
          ...
          applyDiscountToCheck: (orderId, discountInput, onError?) => {
            ...
            const isPositiveAmountsConstraint = (errorLike: any): boolean => {
              ...
              return (
                text.includes("positive_amounts") || text.includes("23514")
              );
            };
              ...
              for (const unsynced of unsyncedDiscounts) {
                const matchingOp = pendingOps.find(
                  (op) =>
                    ...
                    (op.params as any)?.discount?.local_id ===
                      unsynced.local_id,
                );
                if (matchingOp) {
          ...
          discardUnsyncedPayments: (orderId) => {
            ...
            set((state) => {
              ...
              // Reset paid_status — after discarding all pending payments we
              // must revert to Unpaid. Leaving a stale "Partial"/"Paid" from
              // a prior backend sync causes the order to show the wrong badge
              // and makes resolveOutstandingAmount prefer the backend amount_due
              // (which is stale on the order object) over the correct
              // calculated outstanding.
              o.paid_status = "Unpaid" as const;

              // Clear split_payment_path when all payments are discarded.
              // This allows the next payment attempt to start fresh without
              // being locked into a stale split mode.
          ...
          addPaymentToOrder: async ({
            ...
            if (isCashPayment) {
              ...
              if (cashOutstanding > 0 && cardOutstanding > cashOutstanding) {
                ...
                cashSavingsValue = round2(
                  ratio * (cardOutstanding - cashOutstanding),
                );
              }
          ...
          fireActiveOrderToKitchen: () => {
            ...
            // sendNewItemsToKitchen / the table path): wait for in-flight item
            // syncs, re-read the FIRED order (the set() above already switched
            // activeOrderId to a fresh empty order, so re-read by the captured
            // firedOrderId), then send every item that has a db_order_item_id and
            // queue stragglers. Fire-and-forget so this action stays synchronous.
            const firedOrderId = activeOrderId;
            const firedItemIds = new Set(
          ...
          sendNewItemsToKitchen: async () => {
            ...
            // Hardened backend send (mirrors the table path in sendToKitchenEffect):
            // send EVERY batch item that resolved a db_order_item_id during the wait
            // — not just those that had one at press time — and queue stragglers.
            // The old "skip items without a db id at press time, trust the
            // retroactive path" logic silently dropped items whose id landed during
          ...
          sendNewItemsToKitchenForOrder: async (orderId: string) => {
            ...
            // OFFLINE-FIRST: Queue or sync backend operation
            // ================================================================
            // Hardened backend send (mirrors sendNewItemsToKitchen / the table
            // path): wait for in-flight item syncs, re-read the order, then send
            // every batch item that has a db_order_item_id and queue stragglers.
          ...
          voidPayment: async (
            ...
            // Only call backend RPC if this payment has a real db_payment_id
            // (backend UUID). Payments with only a local ID (pending/optimistic,
            // never confirmed) must NOT be sent to void_payment — the RPC will
            // reject the phantom UUID, trigger a rollback, and corrupt state.
            const hasBackendPaymentId =
              supabase && order.db_order_id && !!paymentToVoid.db_payment_id;

            if (hasBackendPaymentId) {
              try {
                ...
                const { error } = await runWithDeadline(
                  ...
                  async (signal) =>
                    await supabase
                      .rpc("void_payment", {
                        p_payment_id: paymentToVoid.db_payment_id,
            ...
            // If no backend sync needed (no db_payment_id), just succeed locally
            toastService.show({
              title: "Payment Voided",
          ...
          voidAllPayments: async (orderId: string): Promise<boolean> => {
            ...
            //
            // CRITICAL: Skip payments that are still pending (no db_payment_id).
            // These are optimistic local-only records that were never confirmed by
            // the backend — calling void_payment RPC with a local UUID would fail,
            // trigger a rollback, and corrupt the state for the already-voided
            // backend-confirmed payments. Pending payments are silently discarded
            // inline after the confirmed voids complete.
            const confirmedPayments: typeof order.payments = [];
            const pendingPayments: typeof order.payments = [];
            for (const p of order.payments) {
              if (p.db_payment_id && !p.isVoided && !p.isPreAuth) {
                confirmedPayments.push(p);
              } else if (!p.isVoided && !p.isPreAuth) {
                pendingPayments.push(p);
              }
            }

            // Void backend-confirmed payments first (reverse order).
            // If any confirmed void fails, stop immediately — the caller
            // handles rollback via voidPayment's internal rollback.
            for (let i = confirmedPayments.length - 1; i >= 0; i--) {
              const p = confirmedPayments[i];
              const success = await get().voidPayment(
                orderId,
                p.db_payment_id!,
              );
              if (!success) {
                return false; // Stop if any void fails — rollback handled inside voidPayment
              }
            }

            // After all confirmed voids succeed, discard pending payments locally.
            // These never reached the backend so no RPC is needed.
            const pendingIds = new Set(pendingPayments.map((p) => p.id));
            if (pendingIds.size > 0) {
              // Aggregate paidQuantity to restore for each item
              const restoreQtyByItemKey = new Map<string, number>();
          ...
          initializeOrders: async (
            ...
            try {
              ...
              set((state) => {
                ...
                state.orderIds = [
                  ...new Set([...preservedIds, ...newOrderIds]),
                ];
                state.tableOrderIdIndex = rebuildTableOrderIdIndex(
                  state.ordersById,
                );
          ...
          recalculateOrder: (orderId: string): OrderTotals => {
            ...
            if (hasItems) {
              ...
              const hasPreAuthPayments = order.payments?.some(
                (p) =>
                  !p.isVoided && (p.isPreAuth || p.status === "authorized"),
              );
            ...
            const detailSyncPromise = (async (): Promise<void> => {
              try {
                ...
                set((state) => {
                  ...
                  const mergedPayments = transformedPayments.map(
                    (serverPmt) => {
              ...
              setTimeout(() => {
                ...
                if (activeOrder?.db_order_id) {
                  store
                    ...
                    .catch((err: any) => {
                      console.warn(
                        "[Rehydrate] Active order sync failed:",
                        err,
                      );
                    });
                }
