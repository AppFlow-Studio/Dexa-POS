  ...
  persist(
      ...
      return {
        ...
        addItemToActiveOrder: (newItem) => {
          ...
          if (!itemToSync.isDraft) {
            const orderToSync = get().orders.find(
        ...
        updateItemInActiveOrder: async (updatedItem) => {
          const { activeOrderId, orders } = get();
          ...
          // Sync to backend if item has been synced (has db_order_item_id)
          const dbOrderItemId =
          ...
          if (dbOrderItemId && _supabaseClient) {
            try {
              if (
                ...
                // Quantity changed - use update_order_item_quantity
                const { data, error } =
                ...
                if (error) {
                  ...
                  toastService.show({
                    title: "Sync Error",
                    message: "Failed to sync quantity change",
                    type: "error",
                  });
                } else {
                  console.log("Quantity synced to backend:", data);
                }
              } else {
                // Other updates - use update_order_item
                const { data, error } = await OrderService.updateOrderItem(
                  _supabaseClient,
                  {
                ...
                if (error) {
                  ...
                  console.log("Item update synced to backend:", data);
                }
              }
            } catch (err) {
              console.error("Error calling update RPC:", err);
            }
          }

          // Recalculate totals after backend sync (or immediately if local)
          recalculateTotals(activeOrderId);
        },
        ...
        removeItemFromActiveOrder: (itemId) => {
          ...
          // If item is already synced (has db_order_item_id), try to void on backend
          if (itemToRemove?.db_order_item_id && _supabaseClient) {
            OrderService.voidOrderItem(
              _supabaseClient,
              itemToRemove.db_order_item_id,
              "User removed"
            ).then(({ error }) => {
              if (error) {
                ...
                toastService.show({
                  title: "Sync Error",
                  message: "Failed to void item on server",
                  type: "error",
                });
              } else {
                console.log(
                  "Item voided on backend:",
                  itemToRemove.db_order_item_id
                );
                ...
                return {
                  ...
                  items: o.items.map((item) => {
                    if (item.id === itemId && item.availableDiscount) {
                      return {
                        ...item,
                        appliedDiscount: item.availableDiscount,
                      };
                    }
                    return item;
                  }),
        ...
        updateOrderStatus: (orderId, status) => {
          ...
          set((state) => ({
            orders: state.orders.map((o) => {
              ...
              const next: Partial<OrderProfile> = {
                order_status: status,
              } as any;
        ...
        consolidateOrdersForTables: (tableIds, tableNames) => {
          ...
          const ordersToMerge = orders.filter(
            (o) =>
              o.service_location_id && tableIds.includes(o.service_location_id)
          );
    ...
    },
    {
      ...
      partialize: (state: OrderState) => ({
        // Only persist orders and activeOrderId
        orders: state.orders,
