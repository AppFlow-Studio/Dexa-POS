  ...
  persist(
    immer((set, get) => ({
      ...
      fetchPreviousSessions: async (supabase, locationId, date) => {
        try {
          const { data, error } = await supabase
            ...
            .select(
              "id, sequence_number, status, total_distributed, data_start_after, data_cutoff_at, approved_at",
            )
      ...
      fetchTipConfig: async (supabase, locationId) => {
        ...
        try {
          ...
          set({
            isLoading: false,
            error: error?.message || "Failed to load tip configuration.",
          });
        }
      },
      ...
      previewDistribution: async (
        ...
        try {
          ...
          set({
            isCalculating: false,
            error: error?.message || "Tip distribution preview failed.",
          });
        }
      },
      ...
      approveDistribution: async (
        ...
        try {
          ...
          if (!sessionId)
            throw new Error("No session_id returned from calculation");
          ...
          set({
            error: error?.message || "Failed to approve tip distribution.",
          });
        }
      },
    })),
    {
      ...
      merge: (persisted: any, current: TipDistributionState) => {
        ...
        if (
          !persisted?._wizardSessionDate ||
          persisted._wizardSessionDate !== _todayStr()
        ) {
          return current;
        }
        return {
