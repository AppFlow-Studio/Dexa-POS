...
export const usePreviousOrdersStore = create<PreviousOrdersState>((set, get) => ({
    previousOrders: [],
    refunds: [],
    ...
    addOrderToHistory: (order: OrderProfile) => {
        ...
        const now = new Date();
        const serialNo = (get().previousOrders.length + 1).toString().padStart(3, '0');

        // Calculate total from items if total_amount is not available
        const calculatedTotal = order.total_amount || order.items.reduce((sum, item) => sum + (item.price * item.quantity), 0);
    ...
    refundFullOrder: (orderId: string, reason: string, refundedBy: string, paymentMethod: PaymentType) => {
    ...
    refundItems: (orderId: string, items: Array<{ itemId: string; quantity: number; reason: string }>, refundedBy: string, paymentMethod: PaymentType) => {
        const order = get().getOrderById(orderId);
        if (!order) {
            return;
        }
            ...
            previousOrders: state.previousOrders.map(o =>
                o.orderId === orderId
                    ? {
                        ...
                        paymentStatus: totalRefunded >= o.total ? "Refunded" : "Partially Refunded"
                    }
                    : o
            )
        }));
