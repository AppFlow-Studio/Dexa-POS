    ...
    return {
                ...
                menuItems: state.menuItems.map((item) => {
                    if (item.id === itemId) {
                        ...
                        if (!currentCategories.includes(categoryName)) {
                            return { ...item, category: [...currentCategories, categoryName] };
                        }
                    }
                    return item;
                }),
            ...
            set((state) => ({
                menuItems: state.menuItems.map((item) => {
                    if (item.id === itemId) {
                        ...
                        return { ...item, category: currentCategories.filter(cat => cat !== categoryName) };
                    }
                    return item;
                }),
            }));
        ...
        getMenuItems: (menuId: string): MenuItemType[] => {
            ...
            return state.menuItems.filter((item: MenuItemType) => {
                const itemCategories = Array.isArray(item.category)
                    ? item.category
                    : (item.category ? [item.category] : []);
                return menu.categories.some(categoryName => itemCategories.includes(categoryName));
            });
        },
        ...
        updateModifierGroup: (id, updates) => {
            set((state) => ({
                modifierGroups: state.modifierGroups.map((modifierGroup) =>
                    modifierGroup.id === id ? { ...modifierGroup, ...updates } : modifierGroup
                ),
            }));
        ...
        deleteModifierGroup: (id) => {
            set((state) => ({
                ...
                modifierGroups: state.modifierGroups.filter((modifierGroup) => modifierGroup.id !== id),
                ...
                menuItems: state.menuItems.map((item) => {
                    ...
                    if (filtered.length !== item.modifiers.length) {
                        return { ...item, modifiers: filtered.length > 0 ? filtered : undefined } as typeof item;
                    }
                    return item;
                }),
            }));
        ...
        getModifierGroup: (id: string): ModifierCategory | undefined => {
            const state = useMenuStore.getState();
            return state.modifierGroups.find((modifierGroup) => modifierGroup.id === id);
        },

        // Scheduling
        setMenuSchedules: (id: string, schedules: Schedule[]) => {
            set((state) => ({
                menus: state.menus.map((m: Menu) => (m.id === id ? { ...m, schedules } : m)),
            }));
        },
        setCategorySchedules: (id: string, schedules: Schedule[]) => {
            set((state) => ({
                categories: state.categories.map((c: Category) => (c.id === id ? { ...c, schedules } : c)),
            }));
        },
        isMenuAvailableNow: (id: string, at?: Date): boolean => {
            const state = get();
