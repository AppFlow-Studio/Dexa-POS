...
interface OrderState {
    ...
    clearCart: () => void;
    voidOrder: (orderId: string, voidReason?: string) => Promise<void>;
            ...
            (set, get) => {
                ...
                return {
                    ...
                    addItemToActiveOrder: (newItem) => {
                        ...
                        const itemWithPrice = { ...newItem, price: effectivePrice };
                        ...
                        const mergeCandidate = updatedItems.find(item => {
                            ...
                            const existingCourse = coursingState.getForOrder(activeOrderId)?.itemCourseMap?.[item.id] ?? 1;
                            if (existingCourse !== currentCourse) return false;
                        ...
                        if (mergeCandidate) {
                            ...
                            coursingState.setItemCourse(activeOrderId, finalItem.id, currentCourse);
                        }
                    ...
                    removeItemFromActiveOrder: (itemId) => {
                        ...
                        // Background sync - use remove for draft/pending, void for confirmed
                        const supabase = _supabaseClient;
                        if (supabase && itemToRemove?.db_order_item_id) {
                            const orderStatus = order.order_status;
                            // Check if order is in draft state (before it's sent to kitchen)
                    ...
                    voidOrder: async (orderId, voidReason = 'Order voided') => {
                        const supabase = _supabaseClient;
                        ...
                        // Call RPC if supabase client is available
                        if (supabase) {
                            try {
                                const { data, error } = await OrderService.voidOrder(
                                    supabase,
                                    orderId,
                                    voidReason
                                );
                                ...
                                if (error) {
                                    ...
                                    // State already updated optimistically, so we continue
                                } else {
                                    console.log('Order voided successfully:', data);
                                }
                            } catch (error) {
                                console.error('Failed to void order:', error);
                                // State already updated optimistically, so we continue
