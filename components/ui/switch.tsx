...
const RGB_COLORS = {
  ...
  dark: {
    primary: "rgb(34, 197, 94)", // green-500
    input: "rgb(63, 63, 70)", // zinc-700 for better visibility in dark mode
  },
} as const;
...
function SwitchNative({
  ...
  return (
    <Animated.View
      ...
      className={cn(
        "h-8 w-[46px] rounded-full justify-center",
        props.disabled && "opacity-50"
      )}
    >
      <SwitchPrimitives.Root
        className={cn(
          "flex-row h-8 w-[46px] shrink-0 items-center rounded-full border-transparent",
          className
        )}
        {...props}
      >
        <Animated.View style={animatedThumbStyle}>
          <SwitchPrimitives.Thumb
            className={
              "h-7 w-7 rounded-full bg-white shadow-md shadow-black/25 ring-0 ml-0.5"
            }
          />
        </Animated.View>
      </SwitchPrimitives.Root>
