...
const ScheduleEditSheet = forwardRef<BottomSheet, Props>(
  function ScheduleEditSheet({ rule, onSave }, ref) {
      ...
      >
        <BottomSheetView className="p-4 h-full">
          ...
          <View className="flex-row gap-4 w-full justify-between">
            <View className="mb-4 flex-1">
              ...
              <View className="rounded-xl p-3 bg-[#1f1f1f]">
                <DateTimePicker
                  ...
                  onChange={(e, date) =>
                    setStart(toHHMM(date || parseHHMM(start)))
                  }
                />
                <Text className="text-white text-center mt-2">
                  {parseHHMM(start).toLocaleTimeString([], {
                    hour: "numeric",
                    minute: "2-digit",
                  })}
                </Text>
            ...
            <View className="mb-6 flex-1">
              ...
              <View className="rounded-xl p-3 bg-[#1f1f1f]">
                ...
                />
                <Text className="text-white text-center mt-2">
                  {parseHHMM(end).toLocaleTimeString([], {
                    hour: "numeric",
                    minute: "2-digit",
                  })}
                </Text>
          ...
          <View className="flex-row gap-3">
            <TouchableOpacity
              onPress={() => {
                /* @ts-ignore */ (ref as any)?.current?.close?.();
              }}
