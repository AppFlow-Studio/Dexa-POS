...
const ModifierScreen = () => {
    ...
    const { addItemToActiveOrder, updateItemInActiveOrder, confirmDraftItem } = useOrderStore();
    ...
    const menuItemForModifiers = (mode === "edit" || (mode === "fullscreen" && cartItem)) && cartItem ?
    ...
    useEffect(() => {
        if (isOpen && currentItem) {
            setQuantity((mode === "edit" || (mode === "fullscreen" && cartItem)) ? cartItem!.quantity : 1);
            setNotes((mode === "edit" || (mode === "fullscreen" && cartItem)) ? cartItem!.customizations.notes || "" : "");
    ...
    useEffect(() => {
        if (isOpen && currentItem && mode !== "edit" && !cartItem) {
            ...
            const { activeOrderId, orders, updateItemInActiveOrder } = useOrderStore.getState();
            const activeOrder = orders.find((o) => o.id === activeOrderId);
            const draftItem = activeOrder?.items.find(item => item.id.startsWith(`draft_${currentItem.id}_`));
    ...
    }, [quantity, modifierSelections, notes, isOpen, currentItem, mode, cartItem]);
    ...
    const handleSave = useCallback(() => {
        ...
        if ((mode === "edit" || (mode === "fullscreen" && cartItem)) && cartItem) {
            ...
            const { activeOrderId, orders, addItemToActiveOrder, removeItemFromActiveOrder } = useOrderStore.getState();
            const activeOrder = orders.find((o) => o.id === activeOrderId);
            const draftItem = activeOrder?.items.find(item => item.id.startsWith(`draft_${currentItem.id}_`));
        ...
        const message = (mode === 'edit' || (mode === 'fullscreen' && cartItem))
            ? 'Updated'
    ...
    const currentCategory = menuItemForModifiers?.modifiers?.find(cat => cat.id === activeCategory);
        ...
        <View className="flex-1 bg-white">
            ...
            <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                ...
                <View className="p-6 border-b border-gray-200">
                    <View className="flex-row items-center gap-4">
                        ...
                        <View className="flex-1">
                            <Text className="text-2xl font-bold text-gray-800">{currentItem.name}</Text>
                            <Text className="text-sm text-gray-600 mt-1">{menuItemForModifiers?.description}</Text>
                ...
                {menuItemForModifiers?.modifiers && menuItemForModifiers.modifiers.length > 0 && (
                    <View className="p-6">
                        <Text className="text-xl font-bold text-gray-800 mb-4">Modifier Options</Text>

                        {/* Modifier Category Cards */}
                        <View className="flex-row flex-wrap gap-3 mb-6">
                            {menuItemForModifiers.modifiers.map((category) => {
                                const hasSelection = Object.values(modifierSelections[category.id] || {}).some(Boolean);
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <Text className={`text-xs ${category.type === "required"
                                            ? "text-red-500"
                                            : "text-gray-500"
                                            }`}>
                            ...
                            <View className="mb-6">
                                <View className="flex-row items-center justify-between mb-4">
                                    ...
                                    <View className="flex-row items-center gap-2">
                                        ...
                                        <Text className="text-sm text-gray-500">
                                            {currentCategory.selectionType === "single" ? "Single select" : "Multiple select"}
                                        </Text>
                                    </View>
                                </View>
                                ...
                                <View className="flex-row flex-wrap gap-3">
                                    {currentCategory.options.map((option) => {
                                        const isSelected = modifierSelections[currentCategory.id]?.[option.id] || false;
                                        ...
                                        return (
                                            <TouchableOpacity
                                                ...
                                                onPress={() => handleModifierToggle(currentCategory.id, option.id)}
                                            ...
                                            >
                                                <Text className={`font-medium text-center ${isSelected
                                                    ? "text-white"
                                                    : isUnavailable
                                                        ? "text-gray-400"
                                                        : "text-gray-800"
                                                ...
                                                {option.price > 0 && (
                                                    <Text className={`text-sm text-center mt-1 ${isSelected ? "text-gray-300" : "text-blue-600"
                                                        }`}>
                ...
                {/* Quantity */}
                <View className="p-6 border-b border-gray-200">
                    <Text className="text-lg font-semibold text-gray-800 mb-4">Quantity</Text>
                ...
                {menuItemForModifiers?.allergens && menuItemForModifiers.allergens.length > 0 && (
                    <View className="p-6 border-b border-gray-200">
                        <Text className="text-lg font-semibold text-gray-800 mb-4">Allergens</Text>
