...
export interface PaymentDetailBodyProps {
  /**
   * Closes the controller's open-duration span on first commit. Optional so
   * the body stays usable outside the controller (tests, storybook).
   */
  onBodyMounted?: () => void;
}

/**
 * The sheet itself. Mounted **only while open** — see the controller in
 * PaymentDetailBottomSheet.tsx, which lazy-imports this module so its ~6.7k
 * lines are neither evaluated at boot nor re-rendered on navigation.
 *
 * Everything below this line assumes an open sheet: the hook graph here is
 * only allowed to run when there is an order to show.
 */
const PaymentDetailBottomSheetBody: React.FC<PaymentDetailBodyProps> = ({
  onBodyMounted,
}) => {
  const { show } = useToast();
  const uiScale = useUiScale();
  ...
  // The BottomSheetMethods handle now lives on the controller, which stays
  // mounted while this body does not — see PaymentDetailBottomSheet.tsx.

  // Closes the controller's open-duration span on the first committed frame.
  useEffect(() => {
    onBodyMounted?.();
    // Mount-only: a later identity change of the callback must not re-report.
  ...
  const handleContinueCharging = useCallback(async () => {
    ...
    if (!getCurrentRoutePath().includes("order-processing")) {
      router.replace("/order-processing");
    }
  }, [orderId, close, show, router]);
...
PaymentDetailBottomSheetBody.displayName = "PaymentDetailBottomSheetBody";

/**
 * Default export so `React.lazy(() => import(...))` in the controller resolves
 * without a `.then(m => ...)` shim.
 */
export default PaymentDetailBottomSheetBody;
