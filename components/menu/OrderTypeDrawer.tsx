...
const OrderTypeDrawer: React.FC<OrderTypeDrawerProps> = ({
    ...
    const { activeOrderId, updateActiveOrderDetails, assignOrderToTable } = useOrderStore();
    const { addCustomer, findCustomerByPhone, incrementOrderCount, searchCustomersByPhone } = useCustomerStore();
    const { tables, updateTableStatus } = useFloorPlanStore();
    const { selectedTable, setSelectedTable, clearSelectedTable } = useDineInStore();
    const activeOrder = useOrderStore(state => state.orders.find(order => order.id === activeOrderId));
    const orderTypes = [
    ...
    const assignCustomerToOrder = (customer: any) => {
        if (activeOrderId) {
            updateActiveOrderDetails({
                ...
                delivery_address: currentOrderType === "Delivery" ? customer.address : undefined,
            });
        }
                ...
                <View className="flex-1 p-4">
                            ...
                            {orderTypes.map((orderType) => (
                                <TouchableOpacity
                                    ...
                                    onPress={() => {
                                        ...
                                        if (activeOrderId) {
                                            updateActiveOrderDetails({ order_type: orderType.value as any });
                                        }
                                        // onClose();
                                    }}
                        ...
                        <View className="mt-4">
                                    ...
                                    {showSuggestions && customerSuggestions.length > 0 && (
                                        <View className="absolute top-full left-0 right-0 bg-[#212121] border border-gray-600 rounded-lg mt-1 max-h-32 z-10">
                                            {customerSuggestions.map((customer) => (
                                                ...
                                                >
                                                    <Text className="text-white font-medium">{customer.name}</Text>
                                                    <Text className="text-gray-400 text-sm">{customer.phoneNumber}</Text>
                                                    {customer.address && (
                                                        <Text className="text-gray-500 text-xs">{customer.address}</Text>
                                                    )}
                                                </TouchableOpacity>
                                            ))}
                                        </View>
                            ...
                            {!isExistingCustomer && (
                                <TouchableOpacity
                                    ...
                                    className="w-full py-3 bg-blue-600 rounded-lg items-center mb-4"
                                >
                            ...
                            {isExistingCustomer && (
                                <View className="w-full py-3 bg-green-600 rounded-lg items-center mb-4">
                                    <Text className="text-white font-semibold">
                    ...
                    {currentOrderType === "Dine In" && (
                        <View className="mt-4 flex-1">
                            ...
                            {/* Selected Table Info */}
                            {selectedTable && (
                                <View className="mb-4 p-3 bg-green-600/20 border border-green-600 rounded-lg">
