...
>(({ tableId, onClose }, ref) => {
  ...
  // Release decoded image bitmaps when leaving the table order screen.
  // TableOrderMenuPanel renders MenuSection → MenuItem → OptimizedListImage →
  // expo-image for every item in the active menu grid. Those bitmaps stay
  // pinned in expo-image's memory cache after decode. Clearing only the MEMORY
  // cache is safe: anything still needed re-decodes on next view.
  useEffect(() => {
    return () => {
      if (__DEV__)
        console.log("[memory] table order unmount → clearMemoryCache()");
  ...
  const handleStartNewCourse = useCallback(async () => {
    ...
    try {
      ...
      show({
        ...
        message:
          "Drop new items here. Previous courses stay open until you Send to Kitchen.",
        type: "success",
      });
    } finally {
    ...
    (courseNumber: number) => {
      ...
      if (!ok) {
        show({
          ...
          message: "Course must be empty and not yet sent to the kitchen.",
          type: "warning",
        });
      }
    },
  ...
  const handleSendCourseToKitchen = useCallback(
    async (course: number, forceResend = false, silent = false) => {
      ...
      if (
        !forceResend &&
        !hasUnsentItems &&
        isCourseSent(activeOrder.id, course)
      ) {
        if (!silent)
          show({
            title: "Already Sent",
  ...
  const handleSendAllToKitchen = useCallback(async () => {
    ...
    const pendingCourses = Array.from(
      new Set(
        unsentItems.map(
          (i) => i.courseNumber ?? state?.itemCourseMap?.[i.id] ?? 1,
        ),
      ),
    ).sort((a, b) => a - b);
