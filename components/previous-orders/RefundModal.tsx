...
const RefundModal: React.FC<RefundModalProps> = ({ isOpen, onClose, order }) => {
    const [refundType, setRefundType] = useState<"full" | "partial">("full");
    const [reason, setReason] = useState("");
    const [selectedItems, setSelectedItems] = useState<Array<{ itemId: string; quantity: number; reason: string }>>([]);
    ...
    const toggleItemSelection = (itemId: string, maxQuantity: number) => {
        ...
        if (existingIndex >= 0) {
            ...
            setSelectedItems(prev => [...prev, { itemId, quantity: maxQuantity, reason: "" }]);
        }
    };
                    ...
                    <View className="mb-6">
                        ...
                        <View className="flex-row gap-3">
                            <TouchableOpacity
                                ...
                                className={`flex-1 py-3 px-4 rounded-lg border-2 ${refundType === "full"
                                    ? "border-blue-500 bg-blue-50"
                                    : "border-gray-300"
                                    }`}
                            >
                                <Text className={`font-semibold text-center ${refundType === "full" ? "text-blue-600" : "text-gray-600"
                                    }`}>
                                    Full Refund
                                </Text>
                            ...
                            <TouchableOpacity
                                ...
                                className={`flex-1 py-3 px-4 rounded-lg border-2 ${refundType === "partial"
                                    ? "border-blue-500 bg-blue-50"
                                    : "border-gray-300"
                                    }`}
                            >
                                <Text className={`font-semibold text-center ${refundType === "partial" ? "text-blue-600" : "text-gray-600"
                                    }`}>
                                    Partial Refund
                                </Text>
                                <Text className="text-sm text-center text-gray-500 mt-1">
                                        ...
                                        <View key={item.id} className="mb-4 p-4 border border-gray-200 rounded-lg">
                                            ...
                                            <View className="flex-row items-center justify-between mb-2">
                                                ...
                                                <TouchableOpacity
                                                    ...
                                                    className={`p-2 rounded-lg ${isSelected ? "bg-blue-500" : "bg-gray-200"
                                                        }`}
                                                >
                                                    {isSelected ? (
                                            ...
                                            {isSelected && (
                                                <View className="space-y-2">
                                                    ...
                                                    <TextInput
                                                        ...
                                                        onChangeText={(text) => updateItemReason(item.id, text)}
