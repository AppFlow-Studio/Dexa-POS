  ...
  if (statusHistory && statusHistory.length > 0) {
    statusHistory.forEach(entry => {
      ...
      events.push({
        ...
        detail:
          entry.notes ||
          (entry.changed_by ? `By: ${entry.changed_by}` : undefined),
    ...
    order.payments.forEach((payment, idx) => {
      ...
      if (payment.tip_adjusted_at) {
        events.push({
          ...
          detail: `New tip: $${payment.tip_amount.toFixed(2)}${
            payment.original_tip_amount != null
              ? ` (was $${payment.original_tip_amount.toFixed(2)})`
              : ''
          }`,
          color: colors.teal
        })
  ...
  if (order.reversals) {
    order.reversals.forEach(reversal => {
      ...
      events.push({
        ...
        detail: `$${reversal.amount.toFixed(2)}${
          reversal.reason_description ? ` - ${reversal.reason_description}` : ''
        }`,
        color: colors.danger
      })
    })
