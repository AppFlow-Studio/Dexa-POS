...
const SeatCourseAccordion: React.FC<SeatCourseAccordionProps> = ({
  ...
  const seatGroups = useMemo(() => {
    ...
    activeOrder?.items?.forEach(item => {
      ...
      if (!groups[seatKey][course])
        groups[seatKey][course] = { items: [], itemCount: 0 }
  ...
  useEffect(() => {
    ...
    if (currentItemCount > prevItemCount.current) {
      setExpandedSeat(
        activeSeat === null || activeSeat === undefined ? 'shared' : activeSeat
      )
    }
        ...
        <Animated.View>
          {sortedSeatKeys.map(seatKey => {
            ...
            const courseGroups =
              ...
              {}
            const totalItems = Object.values(courseGroups).reduce(
            ...
            return (
              <View key={`seat-${seatKey}`} className='mb-1'>
                {/* Seat Header */}
                <TouchableOpacity
                        ...
                        <View key={`course-${course}`} className='mb-1'>
                          ...
                          >
                            <View className='flex-row items-center'>
                              ...
                              {aggregateStatus && (
                                <View
                                  className={`ml-1 px-2 py-0.5 rounded ${BADGE_CONFIG[aggregateStatus].bg}`}
                                >
                                  <Text
                                    className={`text-[10px] font-bold ${BADGE_CONFIG[aggregateStatus].text}`}
                                  >
