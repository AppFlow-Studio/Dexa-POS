...
const BillItem: React.FC<BillItemProps> = ({
  ...
  const { mode, openToView, openFullscreenEdit } = useModifierSidebarStore();
  ...
  const handleItemPress = () => {
    ...
    if (isEditable) {
      ...
      translateX.value = withSpring(newIsVisible ? -DELETE_BUTTON_WIDTH : 0, {
        damping: 15,
        stiffness: 120,
      });
    }
          ...
          <View className="flex-row items-center p-3">
            <View className="flex-1 flex-row items-center">
              ...
              <View className="flex-row items-center gap-x-8">
                <Text className="text-xl text-gray-300 w-16 text-right">
                  x {item.quantity}
                </Text>
                <Text className="font-semibold text-2xl text-white w-24 text-right">
