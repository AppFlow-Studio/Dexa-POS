...
const CashPaymentView = () => {
  ...
  const { setBaseAmount, showPayment, showProcessing, showApproved, showDeclined } = useCFD()
  ...
  const effectiveOutstandingCash =
    activeOrderOutstandingCash > 0
      ...
      : activeOrder?.cash_amount_due !== undefined && activeOrder.cash_amount_due >= 0.01
  ...
  const displayGrandTotal = isProcessing ? frozenGrandTotal.current : totalWithTip
  ...
  const handleProcessCashPayment = async () => {
    ...
    try {
      ...
      await handlePaymentCompletion({
        method: 'Cash',
        tipAmount,
        transactionDetails: { amountTendered: amountTenderedNum, isCashPriced: true }
      })
      showApproved()
  ...
  const changeBreakdown = (() => {
    ...
    const bills = [100, 50, 20, 10, 5, 1, 0.25, 0.10, 0.05, 0.01]
    ...
    for (const bill of bills) {
      ...
      if (count > 0) {
        result.push({
          label: bill >= 1 ? `$${bill.toFixed(0)}` : `${Math.round(bill * 100)}¢`,
          count
        })
    ...
    <View style={{ flex: 1, backgroundColor: colors.screen }}>
      ...
      >
        ...
        >
          ...
          <Text style={{ color: colors.muted, fontSize: 13, fontWeight: '600' }}>Back</Text>
        </TouchableOpacity>

        <Text style={{ flex: 1, textAlign: 'center', fontSize: 15, fontWeight: '700', color: colors.heading }}>
          Cash Payment
        </Text>
      ...
      <View style={{ flex: 1, flexDirection: 'row' }}>
          ...
          <View style={{ gap: 10 }}>
            ...
            <View style={{ gap: 5 }}>
              ...
              <TouchableOpacity
                ...
                style={[inputStyle, { borderColor: activeInput === 'tendered' ? colors.teal : colors.border }]}
              >
                <Text style={{ color: colors.muted, fontSize: 16, fontWeight: '600' }}>$</Text>
                <Text style={{ flex: 1, fontSize: 20, fontWeight: '700', color: colors.heading, marginLeft: 4 }}>
                  {amountTendered || '0.00'}
                </Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={{ gap: 5 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                ...
                <Text style={{ color: colors.muted, fontSize: 10 }}>Goes to server tip-out</Text>
              </View>
              <TouchableOpacity
                onPress={() => setActiveInput('tip')}
                style={[inputStyle, { height: 42, borderColor: activeInput === 'tip' ? colors.teal : colors.border }]}
              >
                <Text style={{ color: colors.muted, fontSize: 15, fontWeight: '600' }}>$</Text>
                <Text style={{ flex: 1, fontSize: 17, fontWeight: '600', color: colors.heading, marginLeft: 4 }}>
                  {cashTip || '0.00'}
                </Text>
              </TouchableOpacity>
            </View>
            ...
            <View
              style={{
                backgroundColor: isSufficient ? `${colors.teal}12` : `${colors.panel}`,
                borderRadius: 10,
                padding: 14,
                borderWidth: 1,
                borderColor: isSufficient ? `${colors.teal}40` : colors.border,
                gap: 8,
                marginTop: 2
              }}
            >
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                >
                  ${isSufficient
                    ...
                    : (grandTotal - tendered > 0 ? (grandTotal - tendered).toFixed(2) : '0.00')}
                </Text>
              </View>
            </View>
            ...
            {changeBreakdown && changeBreakdown.length > 0 && (
              ...
              >
                <Text style={{ fontSize: 10, fontWeight: '700', color: colors.teal, textTransform: 'uppercase', letterSpacing: 0.8 }}>
                  Give back
                </Text>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
                  {changeBreakdown.map(({ label, count }) => (
                    ...
                    >
                      <Text style={{ color: colors.teal, fontWeight: '700', fontSize: 12 }}>{count}×</Text>
                      <Text style={{ color: colors.teal, fontWeight: '600', fontSize: 12 }}>{label}</Text>
                    </View>
                  ))}
                </View>
              </View>
        ...
        {/* ── RIGHT PANEL ── */}
        <View style={{ flex: 1, paddingVertical: 12, paddingHorizontal: 16, justifyContent: 'space-between' }}>
          ...
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                ...
                {row.map(btn => (
                  ...
                  >
                    {btn === '⌫' ? (
                      ...
                      <Text style={{ color: colors.heading, fontSize: 17, fontWeight: '600' }}>
                        {btn}
                      </Text>
                    )}
                  </TouchableOpacity>
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 32 }}>
              ...
              >
                <Text style={{ color: '#000', fontWeight: '700', fontSize: 9, textTransform: 'uppercase', letterSpacing: 0.4 }}>
                  Exact
                </Text>
              ...
              {PRESET_BILLS_ROW1.map(bill => (
                ...
                >
                  <Text style={{ color: colors.heading, fontWeight: '700', fontSize: 12 }}>
                    ${bill}
                  </Text>
                </TouchableOpacity>
              ))}
      ...
      <View style={{
        ...
        >
          ...
          {!isProcessing && <Text style={{ color: '#000', fontSize: 13 }}>→</Text>}
        </TouchableOpacity>
      </View>
      </View>
    </View>
