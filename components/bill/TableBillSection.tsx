...
const TableBillSectionContent = ({ cart, currentCourse, itemCourseMap, sentCourses }: { cart: CartItem[]; currentCourse?: number; itemCourseMap?: Record<string, number>; sentCourses?: Record<number, boolean>; }) => {
    ...
    return (
        <>
            <View className="max-w-96 bg-[#303030] flex-1">
                ...
                {Object.keys(courseSummary).length > 0 && (
                    <View className="px-3 py-2 border-b border-gray-700">
                        <View className="flex-row flex-wrap gap-2">
                            {Object.entries(courseSummary).sort(([a], [b]) => Number(a) - Number(b)).map(([course, count]) => {
                                const sent = !!sentCourses?.[Number(course)];
                                const isActive = Number(course) === (currentCourse ?? 0);
                                return (
                                    <View key={course} className={`px-2 py-1 rounded-full flex-row items-center gap-2 ${sent ? "bg-green-900/30 border border-green-500" : isActive ? "bg-blue-900/30 border border-blue-500" : "bg-[#212121] border border-gray-700"
                                        }`}>
                                        <View className={`w-2 h-2 rounded-full ${sent ? "bg-green-500" : "bg-gray-500"}`} />
                                        <Text className="text-xs font-semibold text-white">Course {course}</Text>
                                        <View className="w-1" />
                                        <Text className="text-xs font-semibold text-gray-300">x{count}</Text>
                                        {sent ? (
                                            <Text className="text-xs font-bold text-green-400 ml-2">Sent</Text>
                                        ) : (
                                            <Text onPress={() => onSelectCourse && onSelectCourse(Number(course))} className="text-xs font-bold text-blue-400 ml-2">Select</Text>
                                        )}
                                    </View>
                                );
                            })}
                ...
                <TableBillSectionContent cart={cart} currentCourse={currentCourse} itemCourseMap={itemCourseMap} sentCourses={sentCourses} />
