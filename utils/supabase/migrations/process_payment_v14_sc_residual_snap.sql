-- =====================================================================
-- Migration: process_payment_v14 — SC snap when payment closes order
-- =====================================================================
-- Wave D follow-up. Forks v13 with one surgical change to the
-- per-payment SC allocation block:
--
...
-- to 'process_payment_v14'.
--
-- Apply AFTER:
--   - process_payment_v13_sc_snapshot.sql
--
-- Rollback: process_payment_v14_sc_residual_snap_rollback.sql (drops
-- v14; client wrappers point back at v13).
-- =====================================================================
...
BEGIN
    ...
    -- ====== v14: bake SC share into v_payment_total for item payments ======
...
COMMENT ON FUNCTION public.process_payment_v14 IS
  'Wave D follow-up to process_payment_v13. Adds residual-snap branch to per-payment service_charge allocation: when v_remaining_after_payment <= 0.05 (this payment closes the order''s outstanding balance), snap v_service_charge_share to v_remaining_sc. Fixes split-by-item / custom-amount under-collection identified in the Wave D review (v13 only snapped on explicit last-split / full-remaining hints). SUM(service_charge) across all payments equals orders.service_charge regardless of split type. Idempotency op: ''process_payment_v14''.';
