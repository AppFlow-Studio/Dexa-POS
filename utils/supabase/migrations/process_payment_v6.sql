...
BEGIN
    ...
    INSERT INTO public.order_payments (
        ...
        CASE WHEN v_is_cash THEN 'Cash payment captured'
             ELSE COALESCE(p_terminal_response->'dejavoo_transaction'->>'resultMessage', 'Approved')
        END,
        v_dejavoo_batch_number,
        ...
        now(),
        now(),
        CASE WHEN COALESCE(p_tip_amount, 0) > 0 THEN now() ELSE NULL END,
        CASE WHEN COALESCE(p_tip_amount, 0) > 0 THEN p_staff_id ELSE NULL END,
        CASE WHEN COALESCE(p_tip_amount, 0) > 0 THEN 0 ELSE NULL END
    )
 ...
 PERFORM log_payment_event(
    ...
    SELECT 
        ...
        COALESCE(SUM(
            ...
            + ROUND((((oi.quantity - COALESCE(oi.paid_quantity, 0)) * oi.unit_price) - ROUND(COALESCE(oi.discount_amount, 0) * (oi.quantity - COALESCE(oi.paid_quantity, 0))::numeric / NULLIF(oi.quantity, 0), 2)) * COALESCE(oi.tax_rate, 0) / 100, 2)
        ), 0),
        COALESCE(SUM(
            ((oi.quantity - COALESCE(oi.paid_quantity, 0)) * oi.cash_price)
            - ROUND(COALESCE(oi.discount_amount, 0) * (oi.quantity - COALESCE(oi.paid_quantity, 0))::numeric / NULLIF(oi.quantity, 0), 2)
            + ROUND((((oi.quantity - COALESCE(oi.paid_quantity, 0)) * oi.cash_price) - ROUND(COALESCE(oi.discount_amount, 0) * (oi.quantity - COALESCE(oi.paid_quantity, 0))::numeric / NULLIF(oi.quantity, 0), 2)) * COALESCE(oi.tax_rate, 0) / 100, 2)
        ), 0)
