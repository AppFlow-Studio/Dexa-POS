-- Add notification_failures column to track SMS send failures
...
CREATE OR REPLACE FUNCTION record_waitlist_sms_result(
  p_waitlist_id UUID,
  p_success     BOOLEAN,
...
-- RPC for re-notifying (allows notified/arrived entries)
CREATE OR REPLACE FUNCTION resend_waitlist_notification(
  p_waitlist_id        UUID,
  p_notification_type  TEXT DEFAULT 'sms'
)
