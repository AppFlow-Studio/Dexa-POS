-- =====================================================================
-- Migration: manage_order_discount_v2 — adds idempotency-key support
-- =====================================================================
-- Forks from manage_order_discount in manage_discounts.sql.
--
-- NOTE: Early-error returns (jsonb {success:false, error:...}) are NOT
-- cached. Only the final successful path caches into v_result. On retry,
-- error paths see the claim row, raise idempotency_in_flight for 60s,
-- then takeover. Acceptable for a low-frequency RPC.
--
-- Rollback: manage_order_discount_v2_rollback.sql
-- =====================================================================
...
BEGIN
  IF p_idempotency_key IS NOT NULL THEN
    v_cached := public._idempotency_claim(p_idempotency_key, 'manage_order_discount_v2');
  ...
  SELECT COALESCE(SUM(oi.subtotal), 0), COALESCE(SUM(oi.cash_subtotal), 0),
