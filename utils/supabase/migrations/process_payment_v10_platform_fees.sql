-- =====================================================================
-- Migration: process_payment_v10 — adds platform-fee tracking
-- =====================================================================
-- Forks process_payment_v9. Five normalized deltas (exception list for
-- scripts/verify_verbatim_diff.sh v9→v10 mode):
--
--   1. New DECLAREs: v_dpp, v_tsp, v_dual_pricing_fee, v_tip_fee,
--      v_cash_equivalent_subtotal_portion, v_items_cash_subtotal.
--
--   2. Fee compute block, after v_subtotal_portion is finalized in each
--      payment-shape branch and before INSERT. The cash-equivalent
--      subtotal is derived to mirror v9's three v_subtotal_portion paths:
--        * Per-item: parallel SUM over the existing payment_calc CTE
--          using cash_price + prorated_cash_discount (always — even on
--          cash payments, since cash payments have v_items_cash_subtotal
--          == v_items_subtotal, yielding fee=0).
--        * Split-portion: scale v_payment_total by cash_subtotal/card_total.
--        * Full/partial: same scale.
--      Tip fee uses the percentage formula on the marked-up tip
--      (frontend pre-multiplies per P1).
--
--   3. INSERT INTO public.order_payments column list and VALUES extend
--      with dual_pricing_fee, tip_fee, dual_pricing_percentage_snapshot,
--      tip_surcharge_percentage_snapshot.
--
--   4. v_result jsonb_build_object extends with the same four keys, so
--      idempotent replay returns them.
--
--   5. _idempotency_claim / _idempotency_complete op string flips from
--      'process_payment_v9' → 'process_payment_v10'. Separate cache
--      namespace; cross-version replay treated as a new request.
--
-- Product decisions enforced inside the body (per source plan):
--   P1 (tip surcharge UX) — frontend bakes surcharge into the displayed
--      tip and sends the marked-up amount as p_tip_amount. Backend
--      extracts the fee silently as p_tip_amount * tsp / (100 + tsp).
--   P2 (force-card-pricing on cash) — fees stay 0 when v_is_cash, even
--      with p_force_card_pricing=true. Guard intentionally on v_is_cash
--      (raw), NOT v_use_cash_pricing (which flips false on the force flag).
--
-- Refunds & tip adjustments are handled in apply_refund_to_payment_v3
-- and adjust_tips_v2.
--
-- Apply AFTER:
--   - 00_idempotency_layer.sql
--   - 00_platform_fee_columns_columns_only.sql
--   - 00_platform_fee_index.sql
--   - process_payment_v9.sql (kept alive for callers still on v9)
--
-- Rollback: process_payment_v10_platform_fees_rollback.sql
-- =====================================================================
...
DECLARE
    ...
    -- v10 additions: platform-fee snapshots + decomposition.
    v_dpp numeric;                                -- dual_pricing_percentage snapshot
    v_tsp numeric;                                -- tip_surcharge_percentage snapshot
    v_dual_pricing_fee numeric := 0;
    v_tip_fee numeric := 0;
    v_cash_equivalent_subtotal_portion numeric := 0;
    v_items_cash_subtotal numeric := 0;           -- per-item branch parallel SUM
BEGIN
    -- v10 delta: idempotency op string is 'process_payment_v10' (separate
    -- namespace from v9). Cross-version replay treated as a new request.
    ...
    IF v_is_item_payment THEN
        ...
        SELECT
            ...
            -- v10 delta: parallel cash-equivalent subtotal sum. Always uses
            -- cash_price + prorated_cash_discount; for cash payments this
            -- equals v_items_subtotal so fee=0 by construction.
            COALESCE(SUM(
        ...
        -- v10 delta: per-item cash equivalent.
        v_cash_equivalent_subtotal_portion := v_items_cash_subtotal;
    ...
    ELSIF v_is_split_payment THEN
        ...
        -- v10 delta: cash equivalent for split payments. For cash, equals
        -- v_subtotal_portion. For card, scales the same payment-share by
        -- cash_subtotal/card_total (denominator stays card_total — the
        -- payment-fraction reference).
        IF v_is_cash THEN
    ...
    ELSE
        ...
        IF v_is_full_remaining THEN
            ...
            -- v10 delta: cash equivalent for full-remaining.
            IF v_is_cash THEN
                v_cash_equivalent_subtotal_portion := v_subtotal_portion;
            ELSIF v_order.card_total > 0 THEN
                v_cash_equivalent_subtotal_portion := ROUND(
        ...
        ELSE
            ...
            -- v10 delta: cash equivalent for partial payments.
            IF v_is_cash THEN
                v_cash_equivalent_subtotal_portion := v_subtotal_portion;
            ELSIF v_order.card_total > 0 THEN
                v_cash_equivalent_subtotal_portion := ROUND(
    ...
    -- v10 delta: pull location-level percentages and compute fees.
    ...
    -- P2: cash payments (v_is_cash, raw) never produce platform fees,
    -- even when p_force_card_pricing=true.
    IF v_is_cash THEN
    ...
    -- Tip fee: percentage formula on marked-up tip (frontend bakes per P1).
 ...
 PERFORM log_payment_event(
    ...
    -- v10 delta: extend v_result with the four new fee fields so
    -- idempotent replay returns them for the client.
