-- =====================================================================
-- Migration: process_payment_v12 — SC residual guard for item payments
-- =====================================================================
-- Bug being closed:
-- When an item payment (p_item_allocations present) covers all
-- non-voided unpaid items, v12 was setting v_order_fully_paid := true
-- regardless of whether the amount captured actually covered the
-- order's cash_total / card_total. Because service charge is an
-- order-level charge (not part of any item's subtotal), an item payment
-- that pays $cash_subtotal + $cash_tax leaves the SC ($4.27 in the
-- repro order) uncollected. v12 then forced amount_due = 0 and the SC
-- silently fell on the floor.
--
-- Forensic trail on staging (order 6c25255a, payment id 5f91d127):
--   payment.amount = $12.25 (cash)
--   payment.subtotal_portion = $11.25, payment.tax_portion = $1.00
--   service_charge_applied.cash_total = $16.52  (SC = $4.27)
--   service_charge_applied.cash_amount_due BEFORE payment = $14.60
--   amount_paid AFTER payment = $12.25  → undercollected by $4.27
--   v_order_fully_paid was set true → amount_due forced to 0 → leak
--
-- This rewrite (in-place CREATE OR REPLACE) makes two surgical changes:
--   1) For item payments, only set v_order_fully_paid = true when
--      all items are covered AND amount_paid (in the appropriate
--      pricing mode) >= total - 0.02 tolerance.
--   2) When item payments cover all items but SC remains, compute
--      amount_due / cash_amount_due directly from (total - paid)
--      instead of relying on the items-based v_unpaid_card_total
--      (which is 0 in this state and would incorrectly hide the SC
--      residual).
--
-- Everything else in v12 is preserved unchanged: defensive
-- apply_service_charge_v1 call, payment row insert, log_payment_event,
-- order_payment_items insert, dpf/tip-fee calcs, sync_version bump,
-- idempotency caching.
--
-- Apply AFTER:
--   - apply_service_charge_v1 (rewritten in 7563167b / 3e96ff66)
--   - process_payment_v12_service_charge_authority (original v12)
--
-- Rollback: process_payment_v12_sc_residual_guard_rollback.sql (re-applies
-- the prior v12 body — keep a copy of the live function before applying).
-- =====================================================================
...
BEGIN
    ...
    -- ====== SC RESIDUAL GUARD — fully-paid determination ======
    -- For item payments: even when all items are covered, the order-level
    -- SC may not be. Verify amount_paid (in the relevant pricing mode)
    -- covers the order's total before declaring it fully paid.
    IF v_is_item_payment THEN
    ...
    -- ====== SC RESIDUAL GUARD — amount_due computation ======
    -- When item payments cover all items but the order isn't fully paid
    -- (because SC residual remains), the items-based v_unpaid_card_total
    -- is 0 — it would incorrectly hide the SC residual. Compute amount_due
    -- directly from total - paid for this case, which surfaces the SC
    -- residual as the actual remaining balance the cashier must collect.
    ...
    ELSIF v_is_item_payment AND v_unpaid_items_count = 0 THEN
        -- Items covered, SC short. Compute residuals directly.
...
COMMENT ON FUNCTION public.process_payment_v12 IS
  'Idempotent payment-processing RPC. Forks v11 with a defensive apply_service_charge_v1(NULL,...) after the orders FOR UPDATE so persisted SC matches current items before payment math runs. SC residual guard: for item payments that cover all items, only declare order fully paid when amount_paid (cash- or card-side per pricing mode) >= total. Otherwise leave amount_due = direct (total - paid) residual so the SC stays collectable. Idempotency op: ''process_payment_v12''.';
