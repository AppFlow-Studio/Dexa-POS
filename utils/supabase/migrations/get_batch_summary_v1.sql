...
--
-- Refund/void discriminator (per project_voided_payment_refunded_amount):
...
--
-- Card brand keys: UPPERCASE per castles-response-mapper.ts normalization
...
--
-- RLS: caller must own the merchant; if a location is on the row, must
-- be in the caller's location scope.
--
-- Rollback: get_batch_summary_v1_rollback.sql
-- =====================================================================
...
BEGIN
  ...
  -- Terminal metadata (best-effort; nulls are fine for the report header)
  ...
  -- ── Scalar aggregates over linked payments ────────────────────────
  -- Sales/refund predicate matches get_platform_fees_summary so totals
  -- reconcile with the platform-fees report.
  WITH src AS (
    SELECT
      p.payment_method,
      p.status,
