-- Adds kiosk_profile_id to the get_location_stations_with_status payload so a
-- self_service station can resolve its linked kiosk_profiles row on the client
-- (see hooks/kiosk/useKioskProfile.ts).
--
-- Replaces the function in-place. All other returned columns are preserved
-- (matches utils/supabase/migrations/get_location_stations_with_receipt_printer.sql).
