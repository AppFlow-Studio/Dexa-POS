-- =====================================================================
-- Wave C — process_payment_v12 (forked from v11)
-- =====================================================================
-- One delta vs v11: a defensive PERFORM apply_service_charge_v1(p_order_id)
-- runs right after the orders FOR UPDATE, before any payment math. This
-- closes the multi-station race the senior backend review flagged for
-- the service-charge feature.
--
-- Race scenario the defensive call closes:
--   T0 station A holds order, SC pinned at $X
--   T1 station B adds an item; client fires applyServiceCharge → SC = $Y
--   T2 station A's applyServiceCharge dispatch fails/queues, never runs
--   T3 station A pays — without the inline call, payment math uses
--      v_order.card_total which is still consistent with $Y (B updated it)
--      but a station-A client that recomputed locally and split the
--      check based on a stale $X would have allocated wrong portions.
//      The inline call snaps SC to the current items + snapshot rate.
--
-- Refresh mode (p_party_size IS NULL) is intentional: process_payment
-- doesn't have party_size in its current signature, and threading it
-- through would mean a Cat-B-style breaking-param wave on every caller.
-- The refresh-mode contract on apply_service_charge_v1 says "trust the
-- existing pinned snapshot and just recompute the $ amount" — which is
-- exactly what we want as a defensive safety net.
--
-- No-cache sub-call: p_idempotency_key passed NULL into the SC RPC. The
-- parent payment idempotency wrap already protects the whole transaction
-- (claim on entry, complete on exit) — if process_payment_v12 retries
-- after a successful prior call, the cached jsonb is returned before any
-- body runs, including the SC UPDATE. No double-application risk.
--
-- All other deltas from v11 are zero. The body is reproduced verbatim
-- (BEGIN_VERBATIM / END_VERBATIM markers preserved) with the SC PERFORM
-- + re-read inserted between the order lock and the payment math, and
-- the idempotency op string flipped 'process_payment_v11' →
-- 'process_payment_v12'.
--
-- Caller migration: services/paymentService.ts must flip its rpc() call
-- from 'process_payment_v11' to 'process_payment_v12' in a follow-up
-- commit. Until that flip lands, this RPC is dormant and v11 stays the
-- live path. v11 is intentionally NOT dropped.
--
-- Apply AFTER:
...
--
-- Rollback: DROP FUNCTION public.process_payment_v12(...). v11 untouched.
-- =====================================================================
...
DECLARE
    ...
    -- v12 additions: SC defensive-refresh result, surfaced in v_result
    -- for client telemetry / debugging without needing a second read.
    v_sc_result jsonb;
BEGIN
    -- v12 delta: idempotency op string is 'process_payment_v12'.
    IF p_idempotency_key IS NOT NULL THEN
        v_cached := public._idempotency_claim(p_idempotency_key, 'process_payment_v12');
        IF v_cached IS NOT NULL THEN
            RETURN v_cached;
        END IF;
    END IF;

    -- BEGIN_VERBATIM (mirrors v11 body; deltas marked v12)
    ...
    -- v12 delta: defensive SC refresh. Refresh-mode (p_party_size NULL)
    -- so the call is a no-op if no rule is pinned, and a recompute-from-
    -- current-items if one is. p_idempotency_key NULL so the SC RPC
    -- doesn't try to claim its own cache row — the parent payment claim
    -- already protects this whole tx (rollback unwinds the SC UPDATE
    -- with it, cached replay short-circuits before this PERFORM runs).
    ...
    -- v12 delta: re-read after SC may have changed card_total/cash_total.
    SELECT * INTO v_order
    FROM public.orders
    WHERE id = p_order_id;
    ...
    IF v_is_item_payment THEN
        ...
        SELECT
            ...
            COALESCE(SUM(
                (pc.qty_paying * pc.cash_price) - pc.prorated_cash_discount
            ), 0),
    ...
    ELSIF v_is_split_payment THEN
        ...
        ELSIF v_order.card_total > 0 THEN
            v_cash_equivalent_subtotal_portion := ROUND(
                v_payment_total * (v_order.cash_subtotal / NULLIF(v_order.card_total, 0)), 2
            );
        END IF;
    ...
    SELECT COALESCE(dual_pricing_percentage, 0),
           COALESCE(tip_surcharge_percentage, 0)
    INTO v_dpp, v_tsp
    ...
    ELSE
        v_dual_pricing_fee := GREATEST(
            0,
            ROUND(v_subtotal_portion - v_cash_equivalent_subtotal_portion, 2)
        );
    END IF;
 ...
 PERFORM log_payment_event(
    ...
    -- v12 delta: include service_charge_applied in v_result so callers can
    -- audit the defensive refresh effect on a single round-trip.
    v_result := jsonb_build_object(
        ...
        'tip_fee', v_tip_fee,
        'dual_pricing_percentage_snapshot', v_dpp,
        'tip_surcharge_percentage_snapshot', v_tsp,

        'acquirer', v_acquirer,
        'batch_number', v_batch_number,

        -- v12 addition: defensive SC refresh result (raw, for debugging).
    ...
    IF p_idempotency_key IS NOT NULL THEN
        ...
        END IF;
        PERFORM public._idempotency_complete(p_idempotency_key, 'process_payment_v12', v_cached_result);
    END IF;
...
GRANT EXECUTE ON FUNCTION public.process_payment_v12(
...
-- =====================================================================
-- How to test (Wave C.2)
-- =====================================================================
-- 1. Apply against staging (dfwqakoyittmrwbqvxgw) ONLY after C.0.5/C.1.
-- 2. Smoke: SELECT proname FROM pg_proc WHERE proname='process_payment_v12';
-- 3. Defensive race rehearsal:
--      a. Open an order via add_open_item with party_size 6, dine_in.
--      b. Call apply_service_charge_v1 with party_size=6 to pin the rule.
--         Note service_charge = $S1, card_total = $T1.
--      c. SIMULATE a missed applyServiceCharge call: directly add another
--         order_items row via INSERT (bypassing the RPC), and recompute:
--             UPDATE orders SET card_subtotal = ..., card_total = ...
--         (i.e. items grew but SC stays stale at $S1.)
--      d. Call process_payment_v12 with the full remaining amount.
--      e. Inspect orders.service_charge AFTER — should be the new $S2
--         derived from the larger subtotal, NOT $S1. The defensive
--         refresh fired before payment math.
-- 4. Idempotency replay — same key twice; second returns cached jsonb
--    (including service_charge_applied), no second SC UPDATE.
-- 5. Manual override coexistence — set service_charge_is_manual=true
--    before payment; process_payment_v12 runs but SC stays as set,
--    service_charge_applied.skipped='manual_override'.
-- 6. Cash-only order, no SC rule — process_payment_v12 runs cleanly,
--    service_charge_applied.eligible=false, service_charge=0.
-- 7. Rollback path: DROP FUNCTION process_payment_v12(...). v11 still
--    serves all live traffic until paymentService.ts is flipped to v12.
-- =====================================================================
