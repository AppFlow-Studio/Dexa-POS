-- =====================================================================
-- Migration: process_payment_v10 — processor-fee pivot (compliance regression)
-- =====================================================================
-- Replaces the v10 body deployed by process_payment_v10_platform_fees.sql.
-- Tip surcharge as a platform markup is non-compliant. This pivot reverts
-- to PURE REPORTING of the bank/processor fee on captured cards.
--
-- Behavior:
--   * Cash payments produce zero fees.
--   * Card payments compute:
--        dual_pricing_fee = subtotal_portion × processor_fee_percentage / 100
--        tip_fee          = tip_amount       × processor_fee_percentage / 100
--   * Snapshot stored: processor_fee_percentage_snapshot.
--   * Card charge and merchant payout are unchanged — the bank deducts
--     its fee from the payout regardless. We track for reconciliation.
--
-- Idempotency op string unchanged: 'process_payment_v10'.
-- Apply AFTER: 00_processor_fee_columns.sql
-- =====================================================================
...
DECLARE
    ...
    v_terminal_id uuid;
    v_processor_fee_pct numeric;
    v_dual_pricing_fee numeric := 0;
    v_tip_fee numeric := 0;
BEGIN
    IF p_idempotency_key IS NOT NULL THEN
        v_cached := public._idempotency_claim(p_idempotency_key, 'process_payment_v10');
    ...
    SELECT COUNT(*),
        COALESCE(SUM(((oi.quantity - COALESCE(oi.paid_quantity, 0) + COALESCE(oi.refunded_quantity, 0)) * oi.unit_price)
        ...
        COALESCE(SUM(((oi.quantity - COALESCE(oi.paid_quantity, 0) + COALESCE(oi.refunded_quantity, 0)) * oi.cash_price)
    ...
    SELECT COALESCE(SUM(CASE WHEN is_cash_priced THEN total_amount ELSE 0 END), 0),
           COALESCE(SUM(CASE WHEN NOT is_cash_priced THEN total_amount ELSE 0 END), 0)
    INTO v_total_cash_paid, v_total_card_paid
    FROM public.order_payments WHERE order_id = p_order_id AND status = 'captured';
    ...
    IF v_is_split_payment THEN
        ...
        FROM public.order_payments WHERE order_id = p_order_id AND split_portion_index IS NOT NULL AND status = 'captured';
    ...
    IF v_is_item_payment THEN
        WITH payment_calc AS (
            SELECT oi.id, oi.item_name, oi.quantity AS original_qty, oi.unit_price, oi.cash_price, oi.tax_rate, oi.discount_amount,
                ...
                ROUND(COALESCE(ROUND(COALESCE(oi.discount_amount, 0) * COALESCE(oi.cash_price, oi.unit_price) / NULLIF(oi.unit_price, 0), 2), 0) *
                    LEAST(COALESCE((alloc.value->>'quantity')::integer, oi.quantity - COALESCE(oi.paid_quantity, 0) + COALESCE(oi.refunded_quantity, 0)),
        ...
        SELECT COALESCE(SUM(CASE WHEN v_is_cash THEN (pc.qty_paying * pc.cash_price) - pc.prorated_cash_discount
                                  ELSE (pc.qty_paying * pc.unit_price) - pc.prorated_discount END), 0),
               COALESCE(SUM(ROUND((CASE WHEN v_is_cash THEN (pc.qty_paying * pc.cash_price) - pc.prorated_cash_discount
                                         ELSE (pc.qty_paying * pc.unit_price) - pc.prorated_discount END) * COALESCE(pc.tax_rate, 0) / 100, 2)), 0),
               array_agg(pc.id)
        INTO v_items_subtotal, v_items_tax, v_covered_items FROM payment_calc pc;
        ...
        SELECT COALESCE(jsonb_agg(jsonb_build_object(
            'order_item_id', pc.id, 'item_name', pc.item_name, 'quantity_paid', pc.qty_paying,
            ...
            'subtotal', CASE WHEN v_is_cash THEN (pc.qty_paying * pc.cash_price) - pc.prorated_cash_discount
                              ELSE (pc.qty_paying * pc.unit_price) - pc.prorated_discount END
        )), '[]'::jsonb) INTO v_covered_items_json FROM payment_calc pc;
    ...
    ELSIF v_is_split_payment THEN
        ...
        IF v_is_last_portion THEN
            v_payment_total := CASE WHEN v_is_cash THEN v_order.cash_total - (v_split_cash_portion * (p_split_count - 1))
                                     ELSE v_order.card_total - (v_split_card_portion * (p_split_count - 1)) END;
            v_payment_total := GREATEST(v_payment_total, 0);
        ELSE
            v_payment_total := CASE WHEN v_is_cash THEN v_split_cash_portion ELSE v_split_card_portion END;
        END IF;
    ...
    ELSE
        ...
        IF v_is_full_remaining THEN
            ...
            SET paid_quantity = quantity, refunded_quantity = 0, refunded_amount = 0,
                price_paid = CASE WHEN v_use_cash_pricing THEN cash_price ELSE unit_price END, updated_at = now()
            WHERE order_id = p_order_id AND is_voided = false
              AND (quantity - COALESCE(paid_quantity, 0) + COALESCE(refunded_quantity, 0)) > 0;
        ELSE
            v_payment_total := LEAST(p_amount, v_target_unpaid); v_payment_total := GREATEST(v_payment_total, 0);
            IF v_use_cash_pricing AND v_order.cash_total > 0 THEN
                v_subtotal_portion := ROUND(v_payment_total * (v_order.cash_subtotal / v_order.cash_total), 2);
            ELSIF v_order.card_total > 0 THEN
                v_subtotal_portion := ROUND(v_payment_total * (v_order.card_subtotal / v_order.card_total), 2);
    ...
    -- Processor (bank) fee compute. Pure reporting — does NOT modify
    -- card charge or merchant payout. Snapshot the rate so historical
    -- rows reflect the rate at capture time.
    SELECT COALESCE(processor_fee_percentage, 0) INTO v_processor_fee_pct
    FROM public.locations WHERE id = v_order.location_id;

    -- Cash payments: zero processor fee (no card processing).
    IF v_is_cash THEN
    ...
    INSERT INTO public.order_payments (
        ...
        is_cash_priced, cash_discount_applied, original_amount,
        covers_items, split_portion_index, split_count, status, terminal_type,
        processed_by_staff_id, processor_response, reference_number,
        transaction_id, authorization_code, dejavoo_response_code,
        dejavoo_batch_number, dejavoo_invoice_number, card_type, card_last_four,
        captured_at, initiated_at, merchant_id, location_id, rrn, result_code,
        result_message, terminal_id, idempotency_key,
        dual_pricing_fee, tip_fee, processor_fee_percentage_snapshot
    ) VALUES (
        ...
        v_dual_pricing_fee, v_tip_fee, v_processor_fee_pct
    ...
    ELSIF v_is_full_remaining AND array_length(v_covered_items, 1) > 0 THEN
        ...
        SELECT v_payment_id, (ci->>'order_item_id')::uuid, (ci->>'quantity_paid')::integer,
               ...
               (ci->>'quantity_paid')::integer * (ci->>'unit_price')::numeric
                 - ROUND(COALESCE((ci->>'discount_amount')::numeric, 0) * (ci->>'quantity_paid')::integer
                         / NULLIF((ci->>'original_quantity')::integer, 0), 2),
    ...
    IF v_is_item_payment THEN v_order_fully_paid := (v_unpaid_items_count = 0);
    ELSIF v_is_split_payment THEN
    ...
    v_result := jsonb_build_object(
        ...
        'change_given', v_change_given, 'is_cash_priced', v_use_cash_pricing, 'pricing_mode', v_new_pricing_mode,
        'is_item_payment', v_is_item_payment, 'is_split_payment', v_is_split_payment,
        'is_full_remaining', v_is_full_remaining, 'split_count', p_split_count,
        'split_portion_index', p_split_portion_index, 'portions_paid', v_portions_paid,
        'portions_remaining', v_portions_remaining, 'split_card_portion', v_split_card_portion,
        'split_cash_portion', v_split_cash_portion, 'items_paid', v_covered_items_json,
        'items_covered', v_covered_items, 'total_cash_paid', v_total_cash_paid,
        'total_card_paid', v_total_card_paid, 'order_amount_paid', v_new_amount_paid,
        'order_amount_due', v_new_amount_due, 'order_cash_amount_due', v_new_cash_amount_due,
        'order_fully_paid', v_order_fully_paid, 'unpaid_items_count', v_unpaid_items_count,
        'unpaid_card_total', v_unpaid_card_total, 'unpaid_cash_total', v_unpaid_cash_total,
    ...
    IF p_idempotency_key IS NOT NULL THEN
        ...
        END IF;
        PERFORM public._idempotency_complete(p_idempotency_key, 'process_payment_v10', v_cached_result);
    END IF;
...
GRANT EXECUTE ON FUNCTION public.process_payment_v10(
...
COMMENT ON FUNCTION public.process_payment_v10 IS
  'Idempotent payment-processing RPC with processor (bank) fee reporting. dual_pricing_fee = subtotal_portion * processor_fee_pct / 100; tip_fee = tip_amount * processor_fee_pct / 100. Cash payments produce zero fees. Pure reporting — never modifies card charge or merchant payout. Idempotency op namespace: process_payment_v10.';
