...
function getSupabase() {
    ...
    if (!supabase) {
        throw new Error("Supabase client not initialized. Ensure the app is connected.");
    }
    return supabase;
}
...
): Promise<{ cardPayment: ProcessPaymentV2Result; cashPayment: ProcessPaymentV2Result }> {
    // 1. Process card portion first
    const cardPayment = await payFullCard(orderId, cardAmount, 0, cardTerminalResponse);
...
export function calculateEvenSplit(
    ...
    const lastPerson = Math.round((total - perPerson * (splitCount - 1)) * 100) / 100;
