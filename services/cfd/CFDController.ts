...
export class CFDController {
    ...
    try {
      ...
      switch (message.type) {
        ...
        case 'phone_submitted':
          if (message.payload) {
            this.callbacks.onPhoneSubmitted?.(
              (message.payload as CFDPhoneResponse).phone
            )
          }
          break
  ...
  updateOrderingPanelImages (
    images: NonNullable<CFDPayload['orderingPanelImages']>
  ): void {
    this.broadcast({
      ...(this.lastPayload as CFDPayload),
  ...
  async unpairClient (clientId: string): Promise<void> {
    ...
    this.server.send(
      clientId,
      JSON.stringify({ type: 'exit_cfd_mode', timestamp: Date.now() })
    )
