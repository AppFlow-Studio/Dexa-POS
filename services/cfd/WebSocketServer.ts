...
import { sha1 } from 'js-sha1' // npm install js-sha1
...
export class WebSocketServer {
    ...
    if (tcpServerEvents) {
      this.subscriptions.push(
        tcpServerEvents.addListener('onClientConnect', ({ clientId }) => {
          this.clients.set(clientId, { id: clientId, isUpgraded: false, buffer: '' })
  ...
  private handleData(clientId: string, data: string): void {
    const client = this.clients.get(clientId)
    if (!client) return
  ...
  private parseFrames(client: WSClient): void {
    const buffer = Buffer.from(client.buffer, 'binary')
    let offset = 0
    ...
    while (offset < buffer.length) {
      ...
      // Unmask if needed
      if (isMasked) {
        const maskKey = buffer.slice(offset + headerLength - 4, offset + headerLength)
    ...
    client.buffer = buffer.slice(offset).toString('binary')
  }
  ...
  private sendFrame(clientId: string, data: Buffer | string, opcode: number = 0x01): void {
  ...
  get clientCount(): number {
    let count = 0
    this.clients.forEach((c) => { if (c.isUpgraded) count++ })
    return count
  }

  get connectedClientIds(): string[] {
    const ids: string[] = []
    this.clients.forEach((c) => { if (c.isUpgraded) ids.push(c.id) })
    return ids
  }
