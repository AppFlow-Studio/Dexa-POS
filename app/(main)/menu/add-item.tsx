...
import { ArrowLeft, Camera, ChevronDown, ChevronUp, Plus, Save, Utensils, X } from "lucide-react-native";
...
const AddMenuItemScreen: React.FC = () => {
    ...
    const availableCategories = categories.filter(cat => cat.isActive).sort((a, b) => a.order - b.order);
    ...
    const validateForm = (): boolean => {
        ...
        if (formData.cashPrice && (isNaN(parseFloat(formData.cashPrice)) || parseFloat(formData.cashPrice) <= 0)) {
        ...
        <View className="flex-1 bg-[#212121]">
            ...
            <View className="flex-1 flex-row">
                ...
                <ScrollView className="flex-1 p-6 bg-[#212121]">
                    <Text className="text-2xl font-bold text-white mb-6">Add New Menu Item</Text>
                    ...
                    <View className="mb-6">
                        ...
                        <View className="mb-4">
                            ...
                            <TextInput
                                className={`bg-[#303030] border rounded-lg px-4 py-3 text-white ${errors.name ? "border-red-500" : "border-gray-600"
                                    }`}
                    ...
                    {/* Pricing */}
                    <View className="mb-6">
                        <Text className="text-lg font-semibold text-white mb-4">Pricing</Text>
                        ...
                        <View className="mb-4">
                            ...
                            {errors.price && <Text className="text-red-400 text-sm mt-1">{errors.price}</Text>}
                        </View>

                        {/* Cash Price */}
                        <View className="mb-4">
                            <Text className="text-white font-medium mb-2">Cash Price (Optional)</Text>
                            <View className="flex-row items-center bg-[#303030] border border-gray-600 rounded-lg px-4 py-3">
                                <Text className="text-white">$</Text>
                                <TextInput
                                    className="flex-1 ml-2 text-white"
                                    ...
                                    onChangeText={(text) => setFormData(prev => ({ ...prev, cashPrice: text }))}
                                    keyboardType="numeric"
                                />
                            </View>
                            {errors.cashPrice && <Text className="text-red-400 text-sm mt-1">{errors.cashPrice}</Text>}
                        </View>
                    </View>
                        ...
                        {availableCategories.length === 0 ? (
                            <View className="bg-[#303030] border border-gray-600 rounded-lg p-4 items-center">
                                ...
                                >
                                    ...
                                    <Text className="text-white font-medium ml-2">Create Category</Text>
                                </TouchableOpacity>
                            </View>
                        ) : (
                            <>
                                <View className="flex-row flex-wrap gap-2">
                                    {availableCategories.map((category) => (
                                        ...
                                        >
                                            <Text className={`font-medium ${formData.categories.includes(category.name) ? "text-white" : "text-gray-300"
                                                }`}>
                                                {category.name}
                                            </Text>
                                        </TouchableOpacity>
                                ...
                                {formData.categories.length > 0 && (
                                    <View className="mt-3">
                                        <Text className="text-gray-400 text-sm mb-2">Selected Categories:</Text>
                                        <View className="flex-row flex-wrap gap-2">
                                            {formData.categories.map((categoryName, index) => (
                                                <View key={index} className="flex-row items-center bg-blue-600/20 border border-blue-500 px-3 py-2 rounded-lg">
                                ...
                                {errors.categories && (
                                    <Text className="text-red-400 text-sm mt-2">{errors.categories}</Text>
                                )}
                            </>
                        )}
                    </View>
                    ...
                    <View className="mb-6">
                        <View className="flex-row items-center justify-between mb-4">
                            <Text className="text-lg font-semibold text-white">Modifier Groups</Text>
                            <TouchableOpacity
                                onPress={() => router.push("/menu/add-modifier")}
                                className="flex-row items-center bg-green-600 px-3 py-1 rounded-lg"
                            >
                                <Plus size={14} color="white" />
                                <Text className="text-white text-sm font-medium ml-1">Add Modifier</Text>
                            </TouchableOpacity>
                        </View>
                        ...
                        {modifierGroups.length === 0 ? (
                            <View className="bg-[#303030] border border-gray-600 rounded-lg p-4 items-center">
                                <Text className="text-gray-400 text-center mb-3">
                                    No modifier groups available. Create one to add customization options.
                                </Text>
                                <TouchableOpacity
                                    onPress={() => router.push("/menu/add-modifier")}
                                    className="flex-row items-center bg-blue-600 px-4 py-2 rounded-lg"
                                >
                                    <Plus size={16} color="white" />
                                    <Text className="text-white font-medium ml-2">Create Modifier Group</Text>
                                </TouchableOpacity>
                            </View>
                        ) : (
                            <>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <View className="flex-row items-center justify-between px-4 py-3">
                                                    <TouchableOpacity onPress={() => toggleModifier(modifier.id)} className="flex-row items-center gap-2 flex-1">
                                                        <Text className={`font-medium ${selected ? "text-white" : "text-gray-300"}`}>
                                                            {modifier.name}
                                                        </Text>
                                                        <View className={`px-2 py-1 rounded-full ${modifier.type === "required" ? "bg-red-900/30 border border-red-500" : "bg-blue-900/30 border border-blue-500"}`}>
                                                            <Text className={`text-xs ${modifier.type === "required" ? "text-red-400" : "text-blue-400"}`}>
                                                                {modifier.type}
                                                            </Text>
                                                        </View>
                                                        <View className="bg-gray-600/30 border border-gray-500 px-2 py-1 rounded-full">
                                                            <Text className="text-xs text-gray-300">
                                                                {modifier.selectionType === "single" ? "Single" : "Multiple"}{modifier.selectionType === "multiple" && modifier.maxSelections ? ` • Max ${modifier.maxSelections}` : ""}
                                                            </Text>
                                                        </View>
                                                    </TouchableOpacity>
                                                    <TouchableOpacity onPress={() => toggleModifierExpand(modifier.id)} className="pl-3 py-1">
                                                ...
                                                {expanded && (
                                                    <View className="px-4 pb-4">
                                                        {modifier.description ? (
                                                            <Text className="text-gray-400 text-xs mb-2">{modifier.description}</Text>
                                                        ) : null}

                                                        {optionPreview.length > 0 && (
                                                            <View>
                                                                <Text className="text-gray-300 text-xs mb-1">Options</Text>
                                                                <View className="gap-1">
                                                                    {optionPreview.map((opt) => (
                                                                        <View key={opt.id} className="flex-row items-center justify-between bg-[#212121] border border-gray-700 rounded-md px-3 py-2">
                                                                            <View className="flex-row items-center gap-2">
                                                                                <Text className="text-white text-xs">{opt.name}</Text>
                                                                                {opt.isDefault ? (
                                                                                    <View className="px-2 py-0.5 rounded-full bg-green-900/30 border border-green-500">
                                                                                        <Text className="text-[10px] text-green-400">Default</Text>
                                                                                    </View>
                                                                                ) : null}
                                                                            </View>
                                                                            <Text className="text-gray-300 text-xs">{opt.price ? `$${opt.price.toFixed(2)}` : "$0.00"}</Text>
                                                                        </View>
                                                                    ))}
                                                                    {modifier.options && modifier.options.length > optionPreview.length ? (
                                                                        <Text className="text-gray-400 text-xs mt-1">+{modifier.options.length - optionPreview.length} more</Text>
                                                                    ) : null}
                                                                </View>
                                                            </View>
                                                        )}
                                ...
                                {formData.modifiers.length > 0 && (
                                    <View className="mt-3">
                                        <Text className="text-gray-400 text-sm mb-2">Selected Modifier Groups:</Text>
                                        <View className="flex-row flex-wrap gap-2">
                    ...
                    <View className="mb-6">
                        ...
                        >
                            ...
                            <View className={`w-6 h-6 rounded-full border-2 ${formData.availability
                        ...
                        <View className="w-full mt-4 gap-y-2">
                                    ...
                                    <View className="flex-row flex-wrap gap-1 mt-1">
                                        {formData.categories.map((category, index) => (
                                            <View key={index} className="bg-blue-900/30 border border-blue-500 px-2 py-1 rounded">
                                                <Text className="text-xs text-blue-400">{category}</Text>
                                            </View>
                                        ))}
                                    </View>
                                ) : (
                                    <Text className="text-gray-500 text-sm mt-1">No categories selected</Text>
                                )}
                            </View>
                                ...
                                <View className="bg-[#212121] p-3 rounded-lg">
                                    ...
                                    <View className="flex-row flex-wrap gap-1 mt-1">
                                        {formData.modifiers.map((modifierId, index) => {
                                            const modifier = modifierGroups.find(m => m.id === modifierId);
                                            return (
                                                <View key={index} className="bg-green-900/30 border border-green-500 px-2 py-1 rounded">
                                                    <Text className="text-xs text-green-400">{modifier?.name}</Text>
                                                </View>
                                            );
                                        })}
                                    </View>
                            ...
                            {formData.description && (
                                <View className="bg-[#212121] p-3 rounded-lg">
                                    ...
                                    <Text className="text-white text-sm mt-1">{formData.description}</Text>
                                </View>
                            )}

                            <View className="bg-[#212121] p-3 rounded-lg">
                                <Text className="text-sm text-gray-400">Status</Text>
                                <Text className={`font-medium ${formData.availability ? "text-green-400" : "text-red-400"
                                    }`}>
                                    {formData.availability ? "Available" : "Unavailable"}
                                </Text>
                            </View>
                    ...
                    <View className="bg-[#303030] rounded-2xl p-6 w-full max-w-sm border border-gray-600">
                        ...
                        <View className="items-center mb-6">
                            ...
                            <Text className="text-gray-400 text-center mt-2">
                                Are you sure you want to add "{formData.name || "this item"}" to the menu?
                            </Text>
                        </View>
