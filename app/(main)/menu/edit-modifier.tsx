...
const EditModifierScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (existing) {
      const initialData = {
        ...
        recipeMap: (existing.options || []).reduce(
          (acc, opt) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (
      ...
      !formData.options.some((o) => o.isDefault === true)
    ) {
      newErrors.options =
    ...
    try {
      ...
      updateModifierGroup(existing.id, {
        ...
        options: formData.options.map((o) => ({
          ...o,
          name: o.name.trim(),
          recipe: formData.recipeMap[o.id],
        })),
      });

      show({
        title: "Modifier Updated",
        message: `"${formData.name.trim()}" has been saved.`,
        type: "success",
      });
      ...
      show({
        title: "Error",
        message: "Failed to update the modifier group.",
        type: "error",
      });
    } finally {
      setIsSaving(false);
    ...
    try {
      ...
      const { data: newGroup, error: groupError } =
        await MenuService.createModifierGroup(supabase, {
          ...
          maxSelections:
            existing.selectionType === "multiple" ? existing.maxSelections : 1,
        });
      if (groupError || !newGroup) {
        show({
          title: "Error",
          message: "Failed to create local copy.",
          type: "error",
        });
        return;
      }
      ...
      const locationItemIds = (existing.items || [])
        .filter(
          (item) => !item.location_id || item.location_id === selectedStore.id,
        )
        .map((item) => item.id);
      ...
      show({
        title: "Local Copy Created",
        message: `"${existing.name}" is now editable for this location.`,
        type: "success",
      });
      ...
      >
        ...
        >
          ...
          >
            <Trash2
              size={s(14)}
              color={canWrite ? colors.danger : colors.muted}
            />
          </TouchableOpacity>
          <TouchableOpacity
            ...
            {formData.options.length === 0 ? (
              ...
              <DraggableFlatList
                ...
                renderItem={({
                  ...
                  const storeOption = existing?.options.find(
                    (o) => o.id === option.id,
                  );
                  const optionOutOfStock = isActivelySnoozed(
                    storeOption?.snoozedUntil,
                  );
                  return (
                    <ScaleDecorator>
                      <View
                      ...
                      >
                        ...
                        <View style={{ flexDirection: "row", gap: s(8) }}>
                          ...
                          <TouchableOpacity
                            onPress={() =>
                              setSelectedOptionIdForRecipe(option.id)
                            }
                            style={{
                              flex: 1,
                              flexDirection: "row",
                              ...
                              borderRadius: s(8),
                              borderWidth: 1,
                              backgroundColor: formData.recipeMap[option.id]
                                ?.length
                                ? colors.teal + "15"
                                : colors.card,
                              borderColor: formData.recipeMap[option.id]?.length
                                ? colors.teal + "40"
                                : colors.border,
                            }}
                          >
                            ...
                            <Text
                              style={{
                                fontSize: s(12),
                                fontWeight: "500",
                                color: formData.recipeMap[option.id]?.length
                                  ? colors.teal
                                  : colors.label,
                              }}
                            >
                              {formData.recipeMap[option.id]?.length
                                ? `${formData.recipeMap[option.id].length} Ingredients`
                                : "Recipe"}
                            </Text>
                          </TouchableOpacity>
                        </View>
