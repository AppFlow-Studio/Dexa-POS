...
import { ArrowLeft, Check, DollarSign, Edit, Minus, Plus, Save, Trash2, Utensils, X } from "lucide-react-native";
                                        ...
                                        >
                                            <View className="flex-row items-center gap-4">
                                                ...
                                                <View className="flex-1">
                                                    ...
                                                    <View className="flex-row items-center gap-2 mt-1">
                                                        ...
                                                        {existing && (
                                                            <Text className="text-yellow-400 text-xs">
                                                                (Category: ${getItemPriceForCategory(item.id, existing.id).toFixed(2)})
                                                            </Text>
                                                        )}
                                                    </View>
                                                    ...
                                                    {existing && item.customPricing && (
                                                        <View className="mt-2">
                                                            {item.customPricing
                                                                ...
                                                                .map((pricing) => (
                                                                    <View key={pricing.id} className="flex-row items-center gap-2 mb-1">
                                                                        {editingPricing?.itemId === item.id && editingPricing.pricing.id === pricing.id ? (
                                                                            // Edit mode
                                                                            <View className="flex-row items-center gap-2 flex-1">
                                                                                <TouchableOpacity
                                                                                    onPress={() => setEditingPricing({
                                                                                        ...editingPricing,
                                                                                        pricing: { ...pricing, price: Math.max(0, pricing.price - 0.25) }
                                                                                    })}
                                                                                    className="p-1"
                                                                                >
                                                                                ...
                                                                                <TextInput
                                                                                    ...
                                                                                    onChangeText={(text) => setEditingPricing({
                                                                                        ...editingPricing,
                                                                                        pricing: { ...pricing, price: parseFloat(text) || 0 }
                                                                                    })}
                                                                                ...
                                                                                <TouchableOpacity
                                                                                    onPress={() => setEditingPricing({
                                                                                        ...editingPricing,
                                                                                        pricing: { ...pricing, price: pricing.price + 0.25 }
                                                                                    })}
                                                                                    className="p-1"
                                                                                >
                                                                                ...
                                                                                <TouchableOpacity
                                                                                    onPress={() => setEditingPricing(null)}
                                                                            ...
                                                                            // View mode
                                                                            <View className="flex-row items-center gap-2 flex-1">
                                                                                <View className={`px-2 py-1 rounded ${pricing.isActive ? 'bg-yellow-900/30 border border-yellow-500' : 'bg-gray-600/30 border border-gray-500'}`}>
                                                                                    <Text className={`text-xs ${pricing.isActive ? 'text-yellow-400' : 'text-gray-400'}`}>
                                                                                        ${pricing.price.toFixed(2)}
                                                                                    </Text>
                                                                                </View>
                                                                                <TouchableOpacity
                                                                                    onPress={() => handleToggleCustomPricingActive(item.id, pricing.id)}
                                                                                    className={`p-1 rounded ${pricing.isActive ? 'bg-green-600' : 'bg-gray-600'}`}
                                                                                >
                                                                                    <Check size={12} color="white" />
                                                                                </TouchableOpacity>
                                                                                <TouchableOpacity
                                                                                    onPress={() => setEditingPricing({ itemId: item.id, pricing })}
                                                                                    className="p-1 bg-blue-600 rounded"
                                                                                >
                                                                                    <Edit size={12} color="white" />
                                                                                </TouchableOpacity>
                                                                                <TouchableOpacity
                                                                                    onPress={() => handleDeleteCustomPricing(item.id, pricing.id)}
                                                    ...
                                                    {existing && (!item.customPricing || !item.customPricing.some(p => p.categoryId === existing.id)) && (
                                                        <TouchableOpacity
                                                        ...
                                                        >
                                                            ...
                                                            <Text className="text-yellow-400 text-xs">Add Custom Price</Text>
                                                        </TouchableOpacity>
                                                    )}
