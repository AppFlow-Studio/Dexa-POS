...
import { ArrowLeft, Check, ChevronDown, ChevronUp, Plus, Save, Utensils, X } from "lucide-react-native";
    ...
    return (
        <View className="flex-1 bg-[#212121]">
            ...
            <ScrollView className="flex-1 p-6">
                <Text className="text-2xl font-bold text-white mb-6">Create New Menu</Text>

                {/* Menu Name Input */}
                <View className="mb-6">
                    <Text className="text-lg font-semibold text-white mb-4">Menu Name</Text>
                    <TextInput
                ...
                <View className="mb-6">
                    <View className="flex-row items-center justify-between mb-4">
                        <Text className="text-lg font-semibold text-white">Select Categories</Text>
                        ...
                        <View className="gap-3">
                            {availableCategories.map((category) => {
                                ...
                                return (
                                    <View
                                        key={category.id}
                                        className={`bg-[#303030] rounded-lg border ${isSelected ? "border-blue-500 bg-blue-900/20" : "border-gray-700"
                                            }`}
                                    >
                                        {/* Main Category Card */}
                                        <TouchableOpacity
                                                        ...
                                                        <View className="flex-row flex-wrap gap-1 w-full">
                                                            {categoryItems.slice(0, 3).map((item, index) => (
                                                                <View key={index} className="bg-gray-600/30 border border-gray-500 px-2 py-1 rounded">
                                                                    <Text className="text-xs text-gray-300">{item.name}</Text>
                                                                </View>
                                                            ))}
                                                            {categoryItems.length > 3 && (
                                                                <TouchableOpacity
                                                                    ...
                                                                    className="bg-blue-600/30 border border-blue-500 px-2 py-1 rounded"
                                                                >
                                            ...
                                            <View className="border-t border-gray-700 p-4 bg-[#2a2a2a]">
                                                ...
                                                <View className="flex-row flex-wrap gap-2 w-full">
                                                    {categoryItems.map((item, index) => (
                                                        <View key={index} className="bg-gray-600/30 border w-[24%] border-gray-500 px-3 py-2 rounded-lg flex-row items-center gap-2">
                                                            {/* Item Image */}
                        ...
                        <View className="bg-[#303030] border border-gray-600 rounded-lg p-4">
                            <View className="flex-row flex-wrap gap-2">
                                {selectedCategories.map((categoryName) => (
                                    <View key={categoryName} className="flex-row items-center bg-blue-600/20 border border-blue-500 px-3 py-2 rounded-lg">
                ...
                <View className="flex-1 bg-black/50 items-center justify-center p-6">
                    <View className="bg-[#303030] rounded-2xl p-6 w-full max-w-sm border border-gray-600">
                        ...
                        <View className="bg-[#212121] rounded-lg p-4 mb-6">
                            <Text className="text-white font-medium mb-2">{menuName}</Text>
                            {menuDescription && (
                                <Text className="text-gray-400 text-sm mb-2">{menuDescription}</Text>
                            )}
                            <Text className="text-gray-400 text-sm">
                                {selectedCategories.length} categories • {totalItems} total items
                            </Text>
                        </View>
