...
import type { PrinterConfig, PrinterRole, PrinterDriverType } from "@/types/printer";
  ...
  return (
    ...
    >
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
        {rightContent}
        {expanded ? <ChevronUp size={16} color={colors.label} /> : <ChevronDown size={16} color={colors.label} />}
      </View>
    </TouchableOpacity>
  );
}
...
const DevicesConnectionsScreen = () => {
  ...
  const [starRoleOverrides, setStarRoleOverrides] = useState<Record<string, "receipt" | "kitchen" | "both">>({});
  ...
  const handleTestConnection = async () => {
    ...
    toastService.show({
      ...
      message: online ? "Connection verified." : "Could not reach terminal. Check network.",
      type: online ? "success" : "error",
    });
  };

  const handleDiagnoseCastles = async () => {
    toastService.show({ title: "Running TCP Diagnostics...", message: "Testing 4 delimiter formats (~35s).", type: "warning" });
    const result = await diagnoseCastlesConnection();
    if (result.dataReceived) {
      toastService.show({ title: "Diagnosis: Data Received!", message: `Delimiter: ${result.delimiterUsed}`, type: "success" });
    } else if (result.tcpConnected) {
      toastService.show({ title: "Diagnosis: TCP OK, No Response", message: result.error ?? "Terminal did not respond", type: "warning" });
    } else {
  ...
  const handleAssignTerminal = async (terminal: (typeof terminals)[number]) => {
    ...
    setIsAssigning(true);
    try {
      await supabase.from("payment_terminals").update({ station_id: null }).eq("station_id", selectedStation.id).neq("id", terminal.id);
      await supabase.from("payment_terminals").update({ is_active: true, station_id: selectedStation.id }).eq("id", terminal.id);
      ...
      setSelectedStation({ ...selectedStation, payment_terminal: newTerminalData });
      toastService.show({ title: "Terminal Switched", message: `Now using ${terminal.name}.`, type: "success" });
      setShowTerminalPicker(false);
    } catch (err) {
      toastService.show({ title: "Assignment Failed", message: err instanceof Error ? err.message : "Failed to switch terminal.", type: "error" });
    } finally {
    ...
    try {
      if (registerFormType === "dejavoo") {
        ...
        if (result.terminalId) {
          ...
          setSelectedStation({
            ...selectedStation,
            payment_terminal: {
              id: result.terminalId, terminal_name: registerForm.name, register_id: registerForm.tpn,
              auth_key: null, terminal_type: "dejavoo", terminal_model: registerForm.model || null,
              is_connected: false, last_connection_status: null, last_connection_test_at: null,
            },
          });
        }
      } else {
        ...
        setSelectedStation({
          ...
          payment_terminal: {
            ...
            last_connection_status: null, last_connection_test_at: null,
          },
        });
      }
      ...
      if (testTargetId) {
        ...
        toastService.show({
          ...
          message: online ? `${registerForm.name} is connected.` : `${registerForm.name} registered but offline.`,
          type: online ? "success" : "warning",
        });
      }

      setShowRegisterForm(false);
      setRegisterForm({ name: "", tpn: "", authKey: "", model: "", environment: "sandbox", ipAddress: "", port: "8080", connectionType: "local_socket" });
    } catch (err) {
      toastService.show({ title: "Registration Failed", message: err instanceof Error ? err.message : "Failed to register terminal.", type: "error" });
    } finally {
  ...
  const handleStartEdit = () => {
    ...
    setEditForm({
      name: currentTerminal.terminal_name || "", model: currentTerminal.terminal_model || "",
      tpn: currentTerminal.register_id || "", authKey: "",
      ipAddress: currentTerminal.ip_address || "", port: String(currentTerminal.port || 8080),
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (currentTerminal.terminal_type === "castles") {
        ...
        if (editForm.authKey.trim()) updatePayload.auth_key = editForm.authKey.trim();
      }

      const { error: dbErr } = await supabase.from("payment_terminals").update(updatePayload).eq("id", currentTerminal.id);
      ...
      setSelectedStation({
        ...
        payment_terminal: {
          ...
          ...(currentTerminal.terminal_type === "castles"
            ? { ip_address: editForm.connectionType === "local_socket" ? editForm.ipAddress.trim() : undefined, port: editForm.connectionType === "local_socket" ? (parseInt(editForm.port, 10) || 8080) : undefined, connection_type: editForm.connectionType === "usb" ? "usb" as const : "local_socket" as const }
      ...
      toastService.show({ title: testResult.success ? "Saved & Online" : "Saved (Offline)", message: testResult.success ? "Settings saved, terminal connected." : "Settings saved but terminal unreachable.", type: testResult.success ? "success" : "warning" });
    } catch (err) {
      toastService.show({ title: "Save Failed", message: err instanceof Error ? err.message : "Failed to save.", type: "error" });
    } finally {
    ...
    try {
      ...
      setAlertModal({
        ...
        message: result.online
          ...
          : `Could not reach ${printer.printerName}. ${result.error || "Check power & network."}`,
      });
    } catch (e: any) {
      setAlertModal({ success: false, title: "Connection Failed", message: e.message || "Unable to connect." });
    } finally {
  ...
  const handleDeletePrinter = (printer: PrinterConfig) => {
    Alert.alert("Delete Printer?", `Remove "${printer.printerName}" permanently?`, [
  ...
  const handleScanStarPrinters = async () => {
    ...
    const countdownId = setInterval(() => {
      setScanSecondsRemaining((prev) => {
        if (prev === null || prev <= 1) { clearInterval(countdownId); return null; }
        return prev - 1;
      });
    }, 1000);
  ...
  const handleProvisionStar = async (discovered: DiscoveredStarPrinter, roleOverride?: "receipt" | "kitchen" | "both") => {
    ...
    try {
      const resolvedOverride = roleOverride ?? starRoleOverrides[discovered.ipAddress];
      const isBoth = resolvedOverride === "both";
      const role: "receipt" | "kitchen" = isBoth ? "receipt" : (resolvedOverride ?? discovered.capabilities.suggestedRole);
      const printerId = await addStarPrinter(supabase, selectedStation.id, selectedStore.id, selectedStore.merchant_id, discovered, role);
      if (printerId) {
        if (isBoth) {
          await updatePrinterConfig(printerId, { isDefaultKitchen: true });
        }
        await fetchPrinters(selectedStore.id);
        toastService.show({ title: "Printer Added", message: `${discovered.modelName} added as ${isBoth ? "receipt + kitchen" : role}.`, type: "success" });
      } else {
        toastService.show({ title: "Failed", message: "Could not add printer.", type: "error" });
      }
    } catch (e: any) {
      toastService.show({ title: "Error", message: e.message || "Provisioning failed", type: "error" });
    } finally {
  ...
  const handleManualIpAdd = async () => {
    ...
    try {
      ...
      const printerId = await addStarPrinter(supabase, selectedStation.id, selectedStore.id, selectedStore.merchant_id, discovered, provisionRole);
      if (printerId) {
        if (isBoth) {
          await updatePrinterConfig(printerId, { isDefaultKitchen: true });
        }
        ...
        setManualIp("");
        setManualIpError(null);
        toastService.show({ title: "Printer Added", message: `${discovered.modelName} at ${ip} added${isBoth ? " as receipt + kitchen" : ""}.`, type: "success" });
      } else {
  ...
  const handleProvisionBuiltin = async () => {
    ...
    try {
      const printerId = await addBuiltinPrinter(supabase, selectedStation.id, selectedStore.id, selectedStore.merchant_id, capabilities);
      if (printerId) {
        await fetchPrinters(selectedStore.id);
        toastService.show({ title: "Built-in Printer Added", message: "Ready to print.", type: "success" });
      }
    } catch (e: any) {
  ...
  const handleProvisionDejavoo = async () => {
    ...
    try {
      const printerId = await addDejavooPrinter(supabase, selectedStation.id, selectedStore.id, selectedStore.merchant_id, currentTerminal);
      if (printerId) {
        await fetchPrinters(selectedStore.id);
        toastService.show({ title: "Dejavoo Printer Added", message: "Terminal printer ready.", type: "success" });
      }
    } catch (e: any) {
      toastService.show({ title: "Error", message: e.message, type: "error" });
    } finally {
  ...
  const hasBuiltinPrinter = scopedPrinters.some((p) => p.printerType === "builtin_landi");
  ...
  const dejavooDetected = currentTerminal?.terminal_type === "dejavoo" && !hasDejavooPrinter;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.screen, paddingHorizontal: 14, paddingVertical: 10 }}>
      <View style={{ marginBottom: 12 }}>
        <Text style={{ fontSize: 16, fontWeight: "700", color: colors.heading }}>Devices & Connections</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={{ backgroundColor: colors.panel, borderRadius: 12, borderWidth: 1, borderColor: colors.border, marginBottom: 12, overflow: "hidden" }}>
          <SectionHeader title="This Station" icon={<Smartphone size={20} color={colors.teal} />} expanded={expandedSections.station} onToggle={() => toggleSection("station")}
            rightContent={
              <TouchableOpacity onPress={handleRefreshCapabilities} disabled={isRefreshingCaps} style={{ padding: 4 }}>
                {isRefreshingCaps ? <ActivityIndicator size="small" color={colors.teal} /> : <RefreshCw size={14} color={colors.teal} />}
              </TouchableOpacity>
            }
          />
              ...
              {capabilities ? (
                <>
                  ...
                  <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
                    {[
                      { label: "Battery", value: capabilities.batteryLevel ? `${capabilities.batteryLevel}%` : "N/A" },
                      { label: "Network", value: capabilities.networkType ?? "N/A" },
                      { label: "IP", value: capabilities.localIpAddress ?? "N/A" },
                      { label: "Version", value: capabilities.appVersion ?? "N/A" },
                    ].map((item) => (
                      <View key={item.label} style={{ backgroundColor: colors.card, borderRadius: 8, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 10, paddingVertical: 6, minWidth: 120 }}>
                        <Text style={{ fontSize: 9, color: colors.muted, fontWeight: "600", textTransform: "uppercase", letterSpacing: 0.5 }}>{item.label}</Text>
                        <Text style={{ fontSize: 12, color: colors.heading, fontWeight: "500", marginTop: 1 }}>{item.value}</Text>
                      </View>
                    ))}
                  </View>
                  <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
                    {[
                      { label: "Built-in Printer", has: capabilities.hasBuiltinPrinter },
                      { label: "CFD", has: capabilities.hasBuiltinCfd },
                      { label: "Cash Drawer", has: capabilities.hasCashDrawerPort },
                      { label: "NFC", has: capabilities.hasNfc },
                      { label: "Scanner", has: capabilities.hasBarcodeScanner },
                    ].filter((f) => f.has).map((f) => (
                      <View key={f.label} style={{ backgroundColor: colors.success + "20", borderWidth: 1, borderColor: colors.success + "40", borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3 }}>
                        <Text style={{ fontSize: 10, fontWeight: "500", color: colors.success }}>{f.label}</Text>
                      </View>
                    ))}
                  </View>
                </>
              ) : (
                <Text style={{ fontSize: 12, color: colors.muted }}>Detecting device capabilities...</Text>
              )}
            </View>
          )}
        </View>
        ...
        {/* ================================================================ */}
        <View style={{ backgroundColor: colors.panel, borderRadius: 12, borderWidth: 1, borderColor: colors.border, marginBottom: 12, overflow: "hidden" }}>
          <SectionHeader title="Payment Terminal" icon={<Radio size={20} color={colors.teal} />} expanded={expandedSections.terminal} onToggle={() => toggleSection("terminal")} />
              ...
              {showRegisterForm ? (
                ...
                <View>
                  ...
                  <View style={{ flexDirection: "row", gap: 10, marginBottom: 12 }}>
                    {(["castles", "dejavoo"] as const).map((type) => (
                      <TouchableOpacity key={type} onPress={() => setRegisterFormType(type)} style={{
                        flex: 1, borderRadius: 8, borderWidth: 1, paddingVertical: 10, paddingHorizontal: 12, alignItems: "center",
                        backgroundColor: registerFormType === type ? colors.teal + "20" : "transparent",
                        borderColor: registerFormType === type ? colors.teal + "50" : colors.border,
                      }}>
                        {type === "castles" ? <Wifi size={18} color={registerFormType === type ? colors.teal : colors.muted} /> : <CreditCard size={18} color={registerFormType === type ? colors.teal : colors.muted} />}
                        <Text style={{ fontWeight: "700", fontSize: 12, marginTop: 6, color: registerFormType === type ? colors.teal : colors.muted }}>{type === "castles" ? "Castles" : "Dejavoo"}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                    ...
                    <>
                      <View style={{ flexDirection: "row", gap: 8, marginBottom: 8 }}>
                        <View style={{ flex: 3 }}>
                          ...
                          <TextInput value={registerForm.ipAddress} onChangeText={(v) => setRegisterForm((f) => ({ ...f, ipAddress: v }))} placeholder="192.168.1.100" placeholderTextColor={colors.muted} keyboardType="decimal-pad" style={{ backgroundColor: colors.screen, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, color: colors.heading, fontSize: 13 }} />
                        </View>
                        <View style={{ flex: 1.2 }}>
                          <Text style={{ color: colors.muted, fontSize: 11, marginBottom: 4 }}>Port</Text>
                          <TextInput value={registerForm.port} onChangeText={(v) => setRegisterForm((f) => ({ ...f, port: v }))} placeholder="8080" placeholderTextColor={colors.muted} keyboardType="number-pad" style={{ backgroundColor: colors.screen, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, color: colors.heading, fontSize: 13 }} />
                        </View>
                      </View>
                      <View style={{ marginBottom: 12 }}>
                        <Text style={{ color: colors.muted, fontSize: 11, marginBottom: 4 }}>Terminal Name *</Text>
                        <TextInput value={registerForm.name} onChangeText={(v) => setRegisterForm((f) => ({ ...f, name: v }))} placeholder="e.g. Front Counter" placeholderTextColor={colors.muted} style={{ backgroundColor: colors.screen, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, color: colors.heading, fontSize: 13 }} />
                      </View>
                    </>
                  ) : (
                    <>
                      {[
                        { key: "name", label: "Terminal Name *", placeholder: "e.g. Front Counter" },
                        { key: "tpn", label: "TPN *", placeholder: "Terminal Point Number" },
                        { key: "authKey", label: "Auth Key *", placeholder: "Authentication Key", secure: true },
                      ].map((field) => (
                        <View key={field.key} style={{ marginBottom: 12 }}>
                          <Text style={{ color: colors.muted, fontSize: 11, marginBottom: 4 }}>{field.label}</Text>
                          <TextInput value={(registerForm as any)[field.key]} onChangeText={(v) => setRegisterForm((f) => ({ ...f, [field.key]: v }))} placeholder={field.placeholder} placeholderTextColor={colors.muted} secureTextEntry={field.secure} style={{ backgroundColor: colors.screen, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, color: colors.heading, fontSize: 13 }} />
                        </View>
                      ))}
                      <View style={{ marginBottom: 12 }}>
                        ...
                        <View style={{ flexDirection: "row", backgroundColor: colors.screen, borderRadius: 8, overflow: "hidden", borderWidth: 1, borderColor: colors.border }}>
                          {(["sandbox", "production"] as const).map((env) => (
                            <TouchableOpacity key={env} onPress={() => setRegisterForm((f) => ({ ...f, environment: env }))} style={{ flex: 1, paddingVertical: 10, alignItems: "center", backgroundColor: registerForm.environment === env ? colors.teal + "20" : "transparent" }}>
                              <Text style={{ fontSize: 13, fontWeight: "500", color: registerForm.environment === env ? colors.teal : colors.muted, textTransform: "capitalize" }}>{env}</Text>
                            </TouchableOpacity>
                          ))}
                        </View>
                      </View>
                  ...
                  <TouchableOpacity onPress={handleRegisterTerminal} disabled={!isRegisterFormValid || isRegistering} style={{
                    ...
                    borderWidth: 1, borderColor: isRegisterFormValid && !isRegistering ? colors.teal + "50" : colors.border,
                  }}>
                    {isRegistering ? <ActivityIndicator size="small" color={colors.teal} /> : (
                      <><Check size={15} color={isRegisterFormValid ? colors.teal : colors.muted} /><Text style={{ fontSize: 13, color: isRegisterFormValid ? colors.teal : colors.muted, fontWeight: "700", marginLeft: 6 }}>{registerFormType === "castles" ? "Save & Connect" : "Register Terminal"}</Text></>
                    )}
                  </TouchableOpacity>
                </View>
              ) : showTerminalPicker ? (
                ...
                <View>
                  <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                    <Text style={{ color: colors.heading, fontWeight: "bold", fontSize: 14 }}>Available Terminals</Text>
                    <TouchableOpacity onPress={() => setShowTerminalPicker(false)}><Text style={{ color: colors.teal }}>Cancel</Text></TouchableOpacity>
                  </View>
                  {terminals.length === 0 ? (
                    <Text style={{ color: colors.muted, textAlign: "center", paddingVertical: 16 }}>No terminals found.</Text>
                  ) : (
                    terminals.map((t) => {
                      const isCurrent = t.id === currentTerminal?.id;
                      const isOtherStation = t.isActive && t.stationId && t.stationId !== selectedStation?.id;
                      return (
                        <TouchableOpacity key={t.id} onPress={() => !isCurrent && !isOtherStation && handleAssignTerminal(t)} disabled={isCurrent || isAssigning || !!isOtherStation}
                          style={{ backgroundColor: isCurrent ? colors.teal + "10" : colors.screen, paddingHorizontal: 12, paddingVertical: 12, borderRadius: 8, marginBottom: 8, flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderWidth: 1, borderColor: isCurrent ? colors.teal + "50" : colors.border, opacity: isOtherStation ? 0.5 : 1 }}>
                          <View style={{ flexDirection: "row", alignItems: "center", flex: 1 }}>
                            <View style={{ width: 10, height: 10, borderRadius: 5, marginRight: 10, backgroundColor: t.isConnected ? colors.success : colors.muted }} />
                            <View style={{ flex: 1 }}>
                              <Text style={{ color: colors.heading, fontWeight: "500", fontSize: 12 }}>{t.name}</Text>
                              <View style={{ flexDirection: "row", alignItems: "center", marginTop: 2 }}>
                                <View style={{ paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, backgroundColor: colors.teal + "30", marginRight: 6 }}>
                                  <Text style={{ fontSize: 10, fontWeight: "500", color: colors.teal }}>{t.terminalType === "castles" ? "Castles" : "Dejavoo"}</Text>
                                </View>
                                {t.model && <Text style={{ color: colors.muted, fontSize: 10 }}>{t.model}</Text>}
                              </View>
                            </View>
                          </View>
                          {isCurrent && <View style={{ backgroundColor: colors.teal + "20", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 4, borderWidth: 1, borderColor: colors.teal + "50" }}><Text style={{ fontSize: 10, color: colors.teal, fontWeight: "700" }}>Current</Text></View>}
                          {isOtherStation && <View style={{ backgroundColor: colors.border, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 4 }}><Text style={{ color: colors.label, fontSize: 10, fontWeight: "bold" }}>In Use</Text></View>}
                        </TouchableOpacity>
                      );
                    })
                  )}
                  <TouchableOpacity onPress={() => { setShowTerminalPicker(false); setShowRegisterForm(true); }} style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", marginTop: 8, paddingVertical: 8 }}>
                    <Plus size={16} color={colors.teal} /><Text style={{ color: colors.teal, fontWeight: "500", marginLeft: 4 }}>Register New Terminal</Text>
                  </TouchableOpacity>
                </View>
                ...
                <View>
                  <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                      ...
                      <View style={{ paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4, backgroundColor: colors.teal + "30" }}>
                        <Text style={{ fontSize: 11, fontWeight: "bold", color: colors.teal }}>{currentTerminal.terminal_type === "castles" ? "Castles" : "Dejavoo"}</Text>
                      </View>
                    </View>
                    <TouchableOpacity onPress={() => setIsEditingTerminal(false)} style={{ padding: 4 }}><X size={18} color={colors.muted} /></TouchableOpacity>
                  </View>

                  <View style={{ marginBottom: 12 }}>
                    <Text style={{ color: colors.muted, fontSize: 11, marginBottom: 4 }}>Terminal Name *</Text>
                    <TextInput value={editForm.name} onChangeText={(v) => setEditForm((f) => ({ ...f, name: v }))} placeholder="e.g. Front Counter" placeholderTextColor={colors.muted} style={{ backgroundColor: colors.screen, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, color: colors.heading, fontSize: 13 }} />
                  </View>
                  ...
                  {currentTerminal.terminal_type === "castles" && (
                    <View style={{ flexDirection: "row", gap: 8, marginBottom: 12 }}>
                      <View style={{ flex: 3 }}>
                        <Text style={{ color: colors.muted, fontSize: 11, marginBottom: 4 }}>IP Address *</Text>
                        <TextInput value={editForm.ipAddress} onChangeText={(v) => setEditForm((f) => ({ ...f, ipAddress: v }))} placeholder="192.168.1.100" placeholderTextColor={colors.muted} keyboardType="decimal-pad" style={{ backgroundColor: colors.screen, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, color: colors.heading, fontSize: 13 }} />
                      </View>
                      <View style={{ flex: 1.2 }}>
                        <Text style={{ color: colors.muted, fontSize: 11, marginBottom: 4 }}>Port</Text>
                        <TextInput value={editForm.port} onChangeText={(v) => setEditForm((f) => ({ ...f, port: v }))} placeholder="8080" placeholderTextColor={colors.muted} keyboardType="number-pad" style={{ backgroundColor: colors.screen, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, color: colors.heading, fontSize: 13 }} />
                      </View>
                    </View>
                  )}
                  {currentTerminal.terminal_type !== "castles" && (
                    <>
                      <View style={{ marginBottom: 12 }}>
                        ...
                        <TextInput value={editForm.tpn} onChangeText={(v) => setEditForm((f) => ({ ...f, tpn: v }))} placeholder="Terminal Point Number" placeholderTextColor={colors.muted} style={{ backgroundColor: colors.screen, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, color: colors.heading, fontSize: 13 }} />
                      </View>
                      <View style={{ marginBottom: 12 }}>
                        <Text style={{ color: colors.muted, fontSize: 11, marginBottom: 4 }}>Auth Key</Text>
                        <TextInput value={editForm.authKey} onChangeText={(v) => setEditForm((f) => ({ ...f, authKey: v }))} placeholder="Leave blank to keep current" placeholderTextColor={colors.muted} secureTextEntry style={{ backgroundColor: colors.screen, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, color: colors.heading, fontSize: 13 }} />
                      </View>
                    </>
                  )}

                  <TouchableOpacity onPress={handleSaveEdit} disabled={!isEditFormValid || isSavingEdit} style={{
                    paddingVertical: 12, borderRadius: 10, alignItems: "center", flexDirection: "row", justifyContent: "center",
                    backgroundColor: isEditFormValid && !isSavingEdit ? colors.teal + "20" : colors.screen,
                    borderWidth: 1, borderColor: isEditFormValid && !isSavingEdit ? colors.teal + "50" : colors.border,
                  }}>
                    {isSavingEdit ? <ActivityIndicator size="small" color={colors.teal} /> : (
                      <><Check size={15} color={isEditFormValid ? colors.teal : colors.muted} /><Text style={{ fontSize: 13, color: isEditFormValid ? colors.teal : colors.muted, fontWeight: "700", marginLeft: 6 }}>Save Changes</Text></>
                    )}
                  </TouchableOpacity>
                </View>
              ) : currentTerminal ? (
                // ── Terminal info card ──
                <View>
                  <View style={{ borderRadius: 12, borderWidth: 1, borderColor: currentTerminal.is_connected ? colors.success + "40" : colors.border, marginBottom: 12, overflow: "hidden" }}>
                    <View style={{ flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", paddingHorizontal: 12, paddingVertical: 10 }}>
                      <View style={{ flex: 1 }}>
                        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                          <Text style={{ color: colors.heading, fontWeight: "700", fontSize: 13 }}>{currentTerminal.terminal_name}</Text>
                          <View style={{ paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, backgroundColor: colors.teal + "20" }}>
                            <Text style={{ fontSize: 9, fontWeight: "700", color: colors.teal }}>{currentTerminal.terminal_type === "castles" ? "CASTLES" : "DEJAVOO"}</Text>
                          </View>
                        </View>
                        <View style={{ marginTop: 4, gap: 2 }}>
                          {currentTerminal.terminal_type === "castles" && currentTerminal.ip_address ? (
                            <Text style={{ color: colors.muted, fontSize: 9, fontFamily: "monospace" }}>{currentTerminal.ip_address}:{currentTerminal.port || 8080}</Text>
                          ) : currentTerminal.register_id ? (
                            <Text style={{ color: colors.muted, fontSize: 9 }}>TPN: {currentTerminal.register_id}</Text>
                          ) : null}
                        </View>
                      </View>
                      <View style={{ flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, backgroundColor: currentTerminal.is_connected ? colors.success + "15" : colors.danger + "15" }}>
                        <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: currentTerminal.is_connected ? colors.success : colors.danger }} />
                        <Text style={{ fontSize: 10, fontWeight: "600", color: currentTerminal.is_connected ? colors.success : colors.danger }}>{currentTerminal.is_connected ? "Online" : "Offline"}</Text>
                      </View>
                    </View>
                  </View>

                  <View style={{ flexDirection: "row", gap: 6 }}>
                    <TouchableOpacity onPress={handleTestConnection} disabled={isTestingConnection} style={{ flex: 1, paddingVertical: 8, borderRadius: 8, alignItems: "center", flexDirection: "row", justifyContent: "center", borderWidth: 1, backgroundColor: colors.teal + "20", borderColor: colors.teal + "50" }}>
                      {isTestingConnection ? <ActivityIndicator size="small" color={colors.teal} /> : <><RefreshCw size={13} color={colors.teal} /><Text style={{ fontWeight: "600", marginLeft: 4, fontSize: 11, color: colors.teal }}>Test</Text></>}
                    </TouchableOpacity>
                    <TouchableOpacity onPress={handleStartEdit} style={{ flex: 1, backgroundColor: colors.teal + "20", borderWidth: 1, borderColor: colors.teal + "50", paddingVertical: 8, borderRadius: 8, alignItems: "center", flexDirection: "row", justifyContent: "center" }}>
                      <Pencil size={13} color={colors.teal} /><Text style={{ color: colors.teal, fontWeight: "600", marginLeft: 4, fontSize: 11 }}>Edit</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => setShowTerminalPicker(true)} style={{ flex: 1, backgroundColor: colors.teal + "20", borderWidth: 1, borderColor: colors.teal + "50", paddingVertical: 8, borderRadius: 8, alignItems: "center", flexDirection: "row", justifyContent: "center" }}>
                      <CreditCard size={13} color={colors.teal} /><Text style={{ color: colors.teal, fontWeight: "600", marginLeft: 4, fontSize: 11 }}>Switch</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => { setShowRegisterForm(true); setQuickTestStatus("idle"); }} style={{ flex: 1, backgroundColor: colors.teal + "20", borderWidth: 1, borderColor: colors.teal + "50", paddingVertical: 8, borderRadius: 8, alignItems: "center", flexDirection: "row", justifyContent: "center" }}>
                      <Plus size={13} color={colors.teal} /><Text style={{ color: colors.teal, fontWeight: "600", marginLeft: 4, fontSize: 11 }}>Add</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              ) : (
                ...
                <View>
                  <View style={{ backgroundColor: colors.screen, borderRadius: 12, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 16, paddingVertical: 16, marginBottom: 16 }}>
                    ...
                    <View style={{ flexDirection: "row", gap: 8, marginBottom: 8 }}>
                      <View style={{ flex: 3 }}>
                        <TextInput value={quickTestIp} onChangeText={(v) => { setQuickTestIp(v); setQuickTestStatus("idle"); }} placeholder="192.168.1.100" placeholderTextColor={colors.muted} keyboardType="decimal-pad" style={{ backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, color: colors.heading, fontSize: 13, fontFamily: "monospace" }} />
                      </View>
                      <View style={{ flex: 1.2 }}>
                        <TextInput value={quickTestPort} onChangeText={setQuickTestPort} placeholder="8080" placeholderTextColor={colors.muted} keyboardType="number-pad" style={{ backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, color: colors.heading, fontSize: 13 }} />
                      </View>
                    </View>
                    <TouchableOpacity onPress={handleQuickTest} disabled={!quickTestIp.trim() || quickTestStatus === "testing"} style={{
                      flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 10, borderRadius: 8, borderWidth: 1,
                      backgroundColor: quickTestStatus === "online" ? colors.success + "15" : quickTestStatus === "offline" ? colors.danger + "15" : colors.teal + "15",
                      borderColor: quickTestStatus === "online" ? colors.success + "50" : quickTestStatus === "offline" ? colors.danger + "50" : colors.teal + "50",
                      opacity: !quickTestIp.trim() ? 0.4 : 1,
                    }}>
                      {quickTestStatus === "testing" ? <><ActivityIndicator size="small" color={colors.teal} /><Text style={{ color: colors.teal, fontSize: 13, marginLeft: 8 }}>Testing...</Text></> :
                       quickTestStatus === "online" ? <><Check size={15} color={colors.success} /><Text style={{ color: colors.success, fontSize: 13, fontWeight: "600", marginLeft: 8 }}>Reachable</Text></> :
                       quickTestStatus === "offline" ? <><WifiOff size={15} color={colors.danger} /><Text style={{ color: colors.danger, fontSize: 13, marginLeft: 8 }}>No response</Text></> :
                       <><Wifi size={15} color={colors.teal} /><Text style={{ color: colors.teal, fontSize: 13, marginLeft: 8 }}>Test Connection</Text></>}
                    </TouchableOpacity>
                  </View>
                  <View style={{ flexDirection: "row", gap: 12 }}>
                    {terminals.length > 0 && (
                      <TouchableOpacity onPress={() => setShowTerminalPicker(true)} style={{ flex: 1, backgroundColor: colors.teal + "20", paddingVertical: 12, borderRadius: 10, flexDirection: "row", alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: colors.teal + "50" }}>
                        <CreditCard size={15} color={colors.teal} /><Text style={{ fontSize: 13, color: colors.teal, fontWeight: "700", marginLeft: 6 }}>Assign Existing</Text>
                      </TouchableOpacity>
                    )}
                    <TouchableOpacity onPress={() => { setShowRegisterForm(true); setQuickTestStatus("idle"); }} style={{ flex: 1, backgroundColor: colors.screen, borderWidth: 1, borderColor: colors.border, paddingVertical: 12, borderRadius: 10, flexDirection: "row", alignItems: "center", justifyContent: "center" }}>
                      <Plus size={15} color={colors.teal} /><Text style={{ fontSize: 13, color: colors.teal, fontWeight: "700", marginLeft: 6 }}>Register New</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              )}
              ...
              <View style={{ flexDirection: "row", marginBottom: 14, gap: 6 }}>
                {(["station", "location"] as const).map((scope) => (
                  <TouchableOpacity key={scope} onPress={() => setPrinterScope(scope)} style={{
                    ...
                    backgroundColor: printerScope === scope ? colors.teal + "20" : "transparent",
                    borderColor: printerScope === scope ? colors.teal + "50" : colors.border,
                  }}>
                    <Text style={{ fontSize: 11, fontWeight: "600", color: printerScope === scope ? colors.teal : colors.muted }}>{scope === "station" ? "This Station" : "All Location"}</Text>
                  </TouchableOpacity>
                ))}
              </View>
        ...
        {/* ================================================================ */}
        <View style={{ backgroundColor: colors.panel, borderRadius: 12, borderWidth: 1, borderColor: colors.border, marginBottom: 12, overflow: "hidden" }}>
          <SectionHeader title="Discovered Devices" icon={<Search size={20} color={colors.teal} />} expanded={expandedSections.discovered} onToggle={() => toggleSection("discovered")}
            rightContent={
              (builtinDetected || dejavooDetected || discoveredStarPrinters.length > 0)
          ...
          {expandedSections.discovered && (
            <View style={{ paddingHorizontal: 12, paddingVertical: 10 }}>
              {/* Scan buttons */}
              <View style={{ flexDirection: "row", gap: 8, marginBottom: 12 }}>
                <TouchableOpacity onPress={handleScanStarPrinters} disabled={isScanningStar} style={{
                  flex: 1, paddingVertical: 10, borderRadius: 8, flexDirection: "row", alignItems: "center", justifyContent: "center",
                  backgroundColor: colors.teal + "20", borderWidth: 1, borderColor: colors.teal + "50",
                }}>
                  {isScanningStar ? (
                    <><ActivityIndicator size="small" color={colors.teal} /><Text style={{ color: colors.teal, fontSize: 12, fontWeight: "600", marginLeft: 6 }}>Scanning...{scanSecondsRemaining ? ` (${scanSecondsRemaining}s)` : ""}</Text></>
                  ) : (
                    <><Search size={14} color={colors.teal} /><Text style={{ color: colors.teal, fontSize: 12, fontWeight: "600", marginLeft: 6 }}>Scan Network</Text></>
                  )}
                </TouchableOpacity>
              </View>
              ...
              {builtinDetected && (
                <View style={{ backgroundColor: colors.card, borderRadius: 10, borderWidth: 1, borderColor: colors.success + "40", padding: 12, marginBottom: 8 }}>
                  <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8, flex: 1 }}>
                      <View style={{ width: 28, height: 28, borderRadius: 7, backgroundColor: colors.success + "20", alignItems: "center", justifyContent: "center" }}>
                        <Printer size={14} color={colors.success} />
                      </View>
                      <View>
                        <Text style={{ fontSize: 12, fontWeight: "600", color: colors.heading }}>Built-in Printer Detected</Text>
                        <Text style={{ fontSize: 10, color: colors.muted }}>{capabilities?.model}</Text>
                      </View>
                    </View>
                    <TouchableOpacity onPress={handleProvisionBuiltin} disabled={provisioningBuiltin} style={{ paddingHorizontal: 12, paddingVertical: 7, borderRadius: 8, backgroundColor: colors.teal + "20", borderWidth: 1, borderColor: colors.teal + "50" }}>
                      {provisioningBuiltin ? <ActivityIndicator size="small" color={colors.teal} /> : <Text style={{ fontSize: 11, fontWeight: "600", color: colors.teal }}>Add</Text>}
                    </TouchableOpacity>
                  </View>
                </View>
              )}
              ...
              {dejavooDetected && (
                <View style={{ backgroundColor: colors.card, borderRadius: 10, borderWidth: 1, borderColor: colors.teal + "40", padding: 12, marginBottom: 8 }}>
                  <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8, flex: 1 }}>
                      <View style={{ width: 28, height: 28, borderRadius: 7, backgroundColor: colors.teal + "20", alignItems: "center", justifyContent: "center" }}>
                        <CreditCard size={14} color={colors.teal} />
                      </View>
                      <View>
                        <Text style={{ fontSize: 12, fontWeight: "600", color: colors.heading }}>Dejavoo Terminal Printer</Text>
                        <Text style={{ fontSize: 10, color: colors.muted }}>{currentTerminal?.terminal_name}</Text>
                      </View>
                    </View>
                    <TouchableOpacity onPress={handleProvisionDejavoo} disabled={provisioningDejavoo} style={{ paddingHorizontal: 12, paddingVertical: 7, borderRadius: 8, backgroundColor: colors.teal + "20", borderWidth: 1, borderColor: colors.teal + "50" }}>
                      {provisioningDejavoo ? <ActivityIndicator size="small" color={colors.teal} /> : <Text style={{ fontSize: 11, fontWeight: "600", color: colors.teal }}>Add</Text>}
                    </TouchableOpacity>
                  </View>
                </View>
              )}
                  ...
                  <View key={discovered.ipAddress} style={{ backgroundColor: colors.card, borderRadius: 10, borderWidth: 1, borderColor: alreadyAdded ? colors.success + "40" : colors.border, padding: 12, marginBottom: 8 }}>
                    <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                      <View style={{ flexDirection: "row", alignItems: "center", gap: 8, flex: 1 }}>
                        ...
                        <View>
                          <Text style={{ fontSize: 12, fontWeight: "600", color: colors.heading }}>{discovered.modelName}</Text>
                          <Text style={{ fontSize: 10, color: colors.muted, fontFamily: "monospace" }}>{discovered.ipAddress}</Text>
                        </View>
                      </View>
                      {alreadyAdded ? (
                        ...
                        <View style={{ flexDirection: "row", gap: 6 }}>
                          <TouchableOpacity onPress={() => handleProvisionStar(discovered, "receipt")} style={{ paddingHorizontal: 10, paddingVertical: 7, borderRadius: 8, backgroundColor: colors.teal + "20", borderWidth: 1, borderColor: colors.teal + "50" }}>
                            <Text style={{ fontSize: 11, fontWeight: "600", color: colors.teal }}>Receipt</Text>
                          </TouchableOpacity>
                          <TouchableOpacity onPress={() => handleProvisionStar(discovered, "kitchen")} style={{ paddingHorizontal: 10, paddingVertical: 7, borderRadius: 8, backgroundColor: colors.teal + "20", borderWidth: 1, borderColor: colors.teal + "50" }}>
                            <Text style={{ fontSize: 11, fontWeight: "600", color: colors.teal }}>Kitchen</Text>
                          </TouchableOpacity>
                          <TouchableOpacity onPress={() => handleProvisionStar(discovered, "both")} style={{ paddingHorizontal: 10, paddingVertical: 7, borderRadius: 8, backgroundColor: colors.teal + "20", borderWidth: 1, borderColor: colors.teal + "50" }}>
                            <Text style={{ fontSize: 11, fontWeight: "600", color: colors.teal }}>Both</Text>
                          </TouchableOpacity>
                        </View>
                      )}
                    </View>
              ...
              {starScanError && <Text style={{ fontSize: 11, color: colors.danger, marginBottom: 8 }}>{starScanError}</Text>}

              {/* Manual IP entry */}
              <View style={{ borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 10, marginTop: 4 }}>
                <Text style={{ fontSize: 11, fontWeight: "600", color: colors.muted, textTransform: "uppercase", marginBottom: 8 }}>Add by IP</Text>
                <View style={{ flexDirection: "row", gap: 8 }}>
                  <TextInput value={manualIp} onChangeText={(v) => { setManualIp(v); setManualIpError(null); }} placeholder="192.168.1.100" placeholderTextColor={colors.muted} keyboardType="decimal-pad" style={{ flex: 3, backgroundColor: colors.screen, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, color: colors.heading, fontSize: 13, fontFamily: "monospace" }} />
                  <View style={{ flexDirection: "row", gap: 4, alignItems: "center" }}>
                    {(["receipt", "kitchen", "both"] as const).map((r) => (
                      <TouchableOpacity key={r} onPress={() => setManualIpRole(r)} style={{
                        paddingHorizontal: 10, paddingVertical: 8, borderRadius: 8, borderWidth: 1,
                        backgroundColor: manualIpRole === r ? colors.teal + "20" : "transparent",
                        borderColor: manualIpRole === r ? colors.teal + "50" : colors.border,
                      }}>
                        <Text style={{ fontSize: 11, fontWeight: "600", color: manualIpRole === r ? colors.teal : colors.muted, textTransform: "capitalize" }}>{r}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                  <TouchableOpacity onPress={handleManualIpAdd} disabled={isProbing || !manualIp.trim()} style={{ paddingHorizontal: 16, paddingVertical: 10, borderRadius: 8, backgroundColor: colors.teal + "20", borderWidth: 1, borderColor: colors.teal + "50", opacity: manualIp.trim() ? 1 : 0.4 }}>
                    {isProbing ? <ActivityIndicator size="small" color={colors.teal} /> : <Text style={{ fontSize: 12, fontWeight: "600", color: colors.teal }}>Add</Text>}
                  </TouchableOpacity>
                </View>
                {manualIpError && <Text style={{ fontSize: 11, color: colors.danger, marginTop: 4 }}>{manualIpError}</Text>}
              </View>
            </View>
          )}
        </View>
      ...
      <Modal visible={!!alertModal} transparent animationType="fade" onRequestClose={() => setAlertModal(null)}>
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.55)", alignItems: "center", justifyContent: "center" }}>
          <View style={{ width: 320, backgroundColor: colors.panel, borderRadius: 14, borderWidth: 1, borderColor: colors.border, overflow: "hidden" }}>
            <View style={{ height: 4, backgroundColor: alertModal?.success ? colors.teal : colors.danger }} />
            <View style={{ padding: 20 }}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 10 }}>
                <View style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: (alertModal?.success ? colors.teal : colors.danger) + "20", alignItems: "center", justifyContent: "center" }}>
                  {alertModal?.success ? <Printer size={16} color={colors.teal} /> : <AlertTriangle size={16} color={colors.danger} />}
                </View>
                <Text style={{ fontSize: 14, fontWeight: "700", color: colors.heading, flex: 1 }}>{alertModal?.title}</Text>
              </View>
              <Text style={{ fontSize: 12, color: colors.label, lineHeight: 18, marginBottom: 20 }}>{alertModal?.message}</Text>
              <TouchableOpacity onPress={() => setAlertModal(null)} style={{ paddingVertical: 9, borderRadius: 8, alignItems: "center", backgroundColor: (alertModal?.success ? colors.teal : colors.danger) + "20", borderWidth: 1, borderColor: (alertModal?.success ? colors.teal : colors.danger) + "50" }}>
                <Text style={{ fontSize: 13, fontWeight: "600", color: alertModal?.success ? colors.teal : colors.danger }}>Dismiss</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
