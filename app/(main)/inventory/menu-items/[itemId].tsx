...
const transactionMeta = (type: string): { label: string; bg: string; text: string } => {
  switch (type) {
    case "PO_RECEIPT":
      return { label: "PO Receipt", bg: colors.success + "20", text: colors.success };
    case "SALES_CONSUMPTION":
      return { label: "Sales/Consumption", bg: colors.teal + "20", text: colors.teal };
    case "SPOILAGE_WASTE":
      return { label: "Spoilage/Waste", bg: colors.danger + "20", text: colors.danger };
    case "INTERNAL_TRANSFER":
      return { label: "Internal Transfer", bg: colors.warning + "20", text: colors.warning };
    case "COUNT_CORRECTION":
      return { label: "Count Correction", bg: colors.info + "20", text: colors.info };
    default:
...
const MenuItemScreen = () => {
  ...
  const generateMockHistory = (id: string) => {
    setInventoryHistory([
      {
        id: "1", itemId: id, type: "PO_RECEIPT",
        quantityChange: 50, resultingQuantity: 50,
        reason: "Initial stock from purchase order",
        timestamp: "2024-01-15T10:30:00Z", userId: "user1", reference: "PO-2024-001",
      },
      {
        id: "2", itemId: id, type: "SALES_CONSUMPTION",
        quantityChange: -5, resultingQuantity: 45,
        reason: "Sales consumption", notes: "Used in 5 orders",
        timestamp: "2024-01-16T14:20:00Z", userId: "user2",
      },
      {
        id: "3", itemId: id, type: "COUNT_CORRECTION",
        quantityChange: 2, resultingQuantity: 47,
        reason: "Count correction", notes: "Found 2 additional units during inventory count",
        timestamp: "2024-01-17T09:15:00Z", userId: "user1",
      },
    ]);
  };
  ...
  const handleSave = () => {
    ...
    const reorderThreshold =
      editStockTrackingMode === "quantity" && editForm.reorderThreshold
        ...
        : undefined;
    setMenuItemStockTrackingMode(item.id, editStockTrackingMode, stockQuantity, reorderThreshold);
    if (editRecipeForm.length > 0) {
      updatedItem.recipe = editRecipeForm
        .filter((r) => r.inventoryItemId && r.quantity)
        .map((r) => ({ inventoryItemId: r.inventoryItemId, quantity: parseFloat(r.quantity) || 0 }));
    }
  ...
  const handleSaveRecipe = () => {
    ...
    const stockQuantity =
      editStockTrackingMode === "quantity" && editForm.stockQuantity
        ? parseInt(editForm.stockQuantity) : undefined;
    const reorderThreshold =
      editStockTrackingMode === "quantity" && editForm.reorderThreshold
        ? parseInt(editForm.reorderThreshold) : undefined;
    setMenuItemStockTrackingMode(item.id, editStockTrackingMode, stockQuantity, reorderThreshold);
  ...
  const handleCancelRecipe = () => {
    ...
    if (item.recipe && item.recipe.length > 0) {
      setEditRecipeForm(item.recipe.map((r) => ({ inventoryItemId: r.inventoryItemId, quantity: r.quantity.toString() })));
    } else {
      setEditRecipeForm([]);
    }
    setIsEditingRecipe(false);
  ...
  const selectInventoryItem = (inventoryItemId: string) => {
    if (editingRecipeItemIndex !== null) {
      setEditRecipeForm((prev) =>
        prev.map((r, i) => i === editingRecipeItemIndex ? { inventoryItemId, quantity: r.quantity } : r)
      );
  ...
  const getInventoryItemName = (id: string) => inventoryItems.find((i) => i.id === id)?.name ?? "Unknown";
  const getInventoryItemUnit = (id: string) => inventoryItems.find((i) => i.id === id)?.unit ?? "units";
  ...
  if (!item) {
    return (
      <View style={{ flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: colors.screen }}>
        <View style={{ width: 48, height: 48, borderRadius: 14, backgroundColor: colors.danger + "20", alignItems: "center", justifyContent: "center", marginBottom: 10 }}>
          <Package size={22} color={colors.danger} />
        </View>
        <Text style={{ fontSize: 14, fontWeight: "700", color: colors.heading, marginBottom: 4 }}>Item Not Found</Text>
        <Text style={{ fontSize: 12, color: colors.muted, marginBottom: 16 }}>This menu item doesn't exist or was removed.</Text>
        <TouchableOpacity
          onPress={() => router.back()}
          style={{ flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 14, paddingVertical: 8, backgroundColor: colors.teal + "20", borderWidth: 1, borderColor: colors.teal + "50", borderRadius: 8 }}
        >
  ...
  const stockTrackingModes: { key: "in_stock" | "out_of_stock" | "quantity"; label: string }[] = [
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.screen }}>
      ...
      >
        <TouchableOpacity
          onPress={() => router.back()}
          style={{ flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 10, paddingVertical: 6, backgroundColor: colors.teal + "15", borderWidth: 1, borderColor: colors.teal + "30", borderRadius: 7 }}
        >
          <ArrowLeft size={14} color={colors.teal} />
          <Text style={{ fontSize: 12, fontWeight: "600", color: colors.teal }}>Back</Text>
        </TouchableOpacity>

        <Text style={{ fontSize: 14, fontWeight: "700", color: colors.heading }}>{item.name}</Text>
          ...
          {isEditing ? (
            <>
              <TouchableOpacity
                ...
                style={{ flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 10, paddingVertical: 6, borderWidth: 1, borderColor: colors.border, borderRadius: 7 }}
              >
              ...
              <TouchableOpacity
                onPress={handleSave}
                style={{ flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 10, paddingVertical: 6, backgroundColor: colors.teal + "20", borderWidth: 1, borderColor: colors.teal + "50", borderRadius: 7 }}
              >
                <Save size={13} color={colors.teal} />
                <Text style={{ fontSize: 12, fontWeight: "600", color: colors.teal }}>Save</Text>
              </TouchableOpacity>
            </>
          ) : (
            <TouchableOpacity
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 14, gap: 12 }}>
          ...
          <View style={{ backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 14 }}>
            <Text style={{ fontSize: 11, fontWeight: "700", color: colors.muted, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 10 }}>Overview</Text>
            <View style={{ gap: 8 }}>
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                <Text style={{ fontSize: 12, color: colors.muted }}>Current Stock</Text>
              ...
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                <Text style={{ fontSize: 12, color: colors.muted }}>Reorder Threshold</Text>
                <Text style={{ fontSize: 13, fontWeight: "600", color: colors.heading }}>
              ...
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                <Text style={{ fontSize: 12, color: colors.muted }}>Availability</Text>
                <View style={{
                  paddingHorizontal: 8, paddingVertical: 3,
                  backgroundColor: item.availability !== false ? colors.success + "20" : colors.danger + "20",
                  borderWidth: 1,
                  borderColor: item.availability !== false ? colors.success + "50" : colors.danger + "50",
                  borderRadius: 6,
                }}>
                  <Text style={{ fontSize: 11, fontWeight: "600", color: item.availability !== false ? colors.success : colors.danger }}>
                    {item.availability !== false ? "Available" : "Unavailable"}
                  </Text>
                </View>
              </View>
              {isLowStock && (
                <View style={{ flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: colors.warning + "15", borderWidth: 1, borderColor: colors.warning + "40", borderRadius: 8, paddingHorizontal: 10, paddingVertical: 7, marginTop: 2 }}>
                  <AlertTriangle size={13} color={colors.warning} />
                  <Text style={{ fontSize: 12, color: colors.warning, fontWeight: "600" }}>Low Stock — reorder soon</Text>
                </View>
              )}
            </View>
          </View>
          ...
          {/* Quick Actions */}
          <View style={{ backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 14 }}>
            <Text style={{ fontSize: 11, fontWeight: "700", color: colors.muted, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 10 }}>Quick Actions</Text>
            <View style={{ flexDirection: "row", gap: 8 }}>
              <TouchableOpacity
                onPress={() => setIsLogUsageModalOpen(true)}
                style={{ flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 8, backgroundColor: colors.teal + "15", borderWidth: 1, borderColor: colors.teal + "40", borderRadius: 8 }}
              >
                <Minus size={14} color={colors.teal} />
                <Text style={{ fontSize: 13, fontWeight: "600", color: colors.teal }}>Log Usage</Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => historySheetRef.current?.expand()}
                style={{ flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 8, borderWidth: 1, borderColor: colors.border, borderRadius: 8 }}
              >
                <History size={14} color={colors.label} />
                <Text style={{ fontSize: 13, fontWeight: "600", color: colors.label }}>View History</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          {/* Item Details */}
          <View style={{ backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 14 }}>
            <Text style={{ fontSize: 11, fontWeight: "700", color: colors.muted, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 12 }}>Item Details</Text>
              ...
              <View style={{ flexDirection: "row", gap: 6 }}>
                {stockTrackingModes.map((mode) => {
                  ...
                  return (
                    ...
                    >
                      <Text style={{ fontSize: 12, fontWeight: "600", color: isActive ? colors.teal : colors.muted }}>
                        {mode.label}
                      </Text>
                    </TouchableOpacity>
                  );
            ...
            <View style={{ flexDirection: "row", gap: 10, marginBottom: 10 }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <TextInput
                  ...
                  onChangeText={(t) => setEditForm((p) => ({ ...p, unitOfMeasure: t }))}
                  editable={isEditing}
            ...
            {/* Price */}
            <View style={{ marginBottom: editStockTrackingMode === "quantity" ? 10 : 0 }}>
              ...
              <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(t) => setEditForm((p) => ({ ...p, stockQuantity: t.replace(/[^0-9.]/g, "") }))}
                    editable={isEditing}
                    keyboardType="numeric"
                    placeholder="0"
                    placeholderTextColor={colors.muted}
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(t) => setEditForm((p) => ({ ...p, reorderThreshold: t.replace(/[^0-9]/g, "") }))}
                    editable={isEditing}
                    keyboardType="numeric"
                    placeholder="0"
                    placeholderTextColor={colors.muted}
          ...
          {/* Recipe Section */}
          <View style={{ backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 14, marginBottom: 20 }}>
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              <Text style={{ fontSize: 11, fontWeight: "700", color: colors.muted, textTransform: "uppercase", letterSpacing: 0.5 }}>Recipe / Ingredients</Text>
              {!isEditingRecipe ? (
                <TouchableOpacity
                ...
                >
                  ...
                  <Text style={{ fontSize: 12, fontWeight: "600", color: colors.teal }}>Edit Recipe</Text>
                </TouchableOpacity>
              ) : (
                <View style={{ flexDirection: "row", gap: 8 }}>
                  <TouchableOpacity
            ...
            {/* Recipe Items */}
            {(isEditingRecipe ? editRecipeForm : item.recipe ?? []).length === 0 ? (
              <View style={{ alignItems: "center", paddingVertical: 20, gap: 6 }}>
                <Text style={{ fontSize: 13, color: colors.muted }}>No ingredients linked yet</Text>
                {isEditingRecipe && (
                  <TouchableOpacity
                    onPress={() => {
                    ...
                    style={{ flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 12, paddingVertical: 7, backgroundColor: colors.teal + "15", borderWidth: 1, borderColor: colors.teal + "30", borderRadius: 7, marginTop: 4 }}
                  >
                    <Plus size={13} color={colors.teal} />
                    <Text style={{ fontSize: 12, fontWeight: "600", color: colors.teal }}>Add Ingredient</Text>
                  </TouchableOpacity>
                )}
              </View>
            ) : (
              <View style={{ gap: 6 }}>
                {(isEditingRecipe ? editRecipeForm : item.recipe ?? []).map((r, index) => (
                  <View
                    key={index}
                    style={{ flexDirection: "row", alignItems: "center", backgroundColor: colors.screen, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8, gap: 10 }}
                  >
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontSize: 13, fontWeight: "600", color: colors.heading }}>
                    ...
                    {isEditingRecipe ? (
                      <>
                        <TextInput
                          value={"quantity" in r ? r.quantity : r.quantity.toString()}
                          onChangeText={(t) => {
                            setEditRecipeForm((prev) =>
                              prev.map((item, i) => i === index ? { ...item, quantity: t.replace(/[^0-9.]/g, "") } : item)
                            );
                          }}
                          keyboardType="numeric"
                          style={{ width: 60, height: 38, backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, borderRadius: 6, color: colors.heading, fontSize: 13, textAlign: "center", paddingVertical: 0 }}
                        />
                        <TouchableOpacity
                          onPress={() => {
                        ...
                        <TouchableOpacity
                          onPress={() => setEditRecipeForm((prev) => prev.filter((_, i) => i !== index))}
                          style={{ padding: 5 }}
                        >
                ...
                {isEditingRecipe && (
                  <TouchableOpacity
                    ...
                    style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 8, borderWidth: 1, borderColor: colors.teal + "40", borderRadius: 8, borderStyle: "dashed" }}
                  >
                    <Plus size={13} color={colors.teal} />
                    <Text style={{ fontSize: 12, fontWeight: "600", color: colors.teal }}>Add Ingredient</Text>
                  </TouchableOpacity>
      ...
      >
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
          <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "center", alignItems: "center", paddingHorizontal: 24 }}>
            <View style={{ backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, borderRadius: 14, padding: 18, width: "100%", maxWidth: 440, gap: 12 }}>
              <Text style={{ fontSize: 15, fontWeight: "700", color: colors.heading }}>Log Usage — {item.name}</Text>
              ...
              <View>
                ...
                <TextInput
                  ...
                  onChangeText={(t) => setLogUsageForm((p) => ({ ...p, quantityUsed: t }))}
                ...
                <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
                  {LOG_REASONS.map((r) => {
                    ...
                    return (
                      <TouchableOpacity
                        key={r.key}
                        onPress={() => setLogUsageForm((p) => ({ ...p, reason: r.key }))}
                        style={{
              ...
              <View>
                ...
                <TextInput
                  ...
                  onChangeText={(t) => setLogUsageForm((p) => ({ ...p, notes: t }))}
                  placeholder="Additional details..."
                  placeholderTextColor={colors.muted}
                  multiline
                  numberOfLines={3}
                  style={[inputStyle(true), { height: 70, paddingTop: 10, textAlignVertical: "top" }]}
                />
              </View>
          ...
          <BottomSheetFlatList
              ...
              return (
                <View style={{ paddingHorizontal: 14, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: colors.border }}>
                  <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                    <View style={{ paddingHorizontal: 8, paddingVertical: 3, backgroundColor: meta.bg, borderRadius: 5 }}>
                      <Text style={{ fontSize: 11, fontWeight: "600", color: meta.text }}>{meta.label}</Text>
                    </View>
                    <Text style={{ fontSize: 13, fontWeight: "700", color: isPositive ? colors.success : colors.danger }}>
                      {isPositive ? "+" : ""}{tx.quantityChange}
                    </Text>
                  </View>
                  <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                    <Text style={{ fontSize: 12, color: colors.muted }}>
                      {new Date(tx.timestamp).toLocaleDateString()} · Resulting: {tx.resultingQuantity}
                    </Text>
                    {tx.reference && (
                      <Text style={{ fontSize: 11, color: colors.muted }}>{tx.reference}</Text>
                    )}
                  </View>
                  {tx.notes && (
                    <Text style={{ fontSize: 11, color: colors.muted, marginTop: 3 }}>{tx.notes}</Text>
                  )}
                </View>
              );
            }}
            ListEmptyComponent={
              <View style={{ alignItems: "center", paddingVertical: 40 }}>
                <Text style={{ fontSize: 13, color: colors.muted }}>No history recorded yet</Text>
              </View>
            }
          />
        </View>
          ...
          <View style={{ flex: 1 }}>
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 14, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: colors.border }}>
              <Text style={{ fontSize: 14, fontWeight: "700", color: colors.heading }}>
                {editingRecipeItemIndex !== null ? "Replace Ingredient" : "Add Ingredient"}
              </Text>
            ...
            <BottomSheetFlatList
              ...
              contentContainerStyle={{ paddingBottom: 20 }}
              renderItem={({ item: inv }) => {
                const isAlreadyAdded = editRecipeForm.some((r) => r.inventoryItemId === inv.id);
                const isCurrentlySelected =
                  editingRecipeItemIndex !== null &&
                  editRecipeForm[editingRecipeItemIndex]?.inventoryItemId === inv.id;
                return (
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      opacity: isAlreadyAdded && !isCurrentlySelected ? 0.45 : 1,
                      backgroundColor: isCurrentlySelected ? colors.teal + "10" : "transparent",
                    }}
                  >
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontSize: 13, fontWeight: "600", color: colors.heading }}>{inv.name}</Text>
                    ...
                    {isCurrentlySelected ? (
                      ...
                      <View style={{ paddingHorizontal: 8, paddingVertical: 3, backgroundColor: colors.muted + "20", borderRadius: 5 }}>
                        <Text style={{ fontSize: 11, color: colors.muted }}>Added</Text>
                      </View>
                    ) : (
