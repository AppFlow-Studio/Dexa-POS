...
const VendorRow: React.FC<{
  ...
  return (
    <View className="flex-row items-center p-6 border-b border-gray-700">
      ...
      <Text className="w-[25%] text-2xl text-gray-300">
        {item.contactPerson}
      </Text>
      <Text className="w-[25%] text-2xl text-gray-300">{item.email}</Text>
    ...
    <View className="flex-1">
      <View className="flex-row justify-between items-center mb-4">
        ...
        >
          ...
          <Text className="text-2xl font-bold text-white">Add Vendor</Text>
        </TouchableOpacity>
      </View>
      ...
      <View className="flex-1 bg-[#303030] border border-gray-700 rounded-xl">
        <View className="flex-row p-6 bg-gray-800/50 rounded-t-xl border-b border-gray-700">
          {TABLE_HEADERS.map((header) => (
            <Text
              key={header}
              className={`font-bold text-xl text-gray-400 ${header === "Vendor Name" ? "w-[20%]" : header === "" ? "w-[10%]" : "w-[25%]"}`}
            >
              {header}
            </Text>
          ))}
