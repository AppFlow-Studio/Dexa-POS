...
import React, { useMemo, useState } from "react"; // <-- MODIFIED: Imported useMemo
  ...
  return (
    <View className="flex-1 bg-[#303030] border border-gray-700 rounded-xl">
      <View className="flex-row p-4 bg-gray-800/50 rounded-t-xl border-b border-gray-700">
        {headers.map((h) => (
          <Text key={h} className="w-1/6 font-bold text-sm text-gray-400">
            {h}
          </Text>
        ))}
      </View>
      <FlatList
        ...
        renderItem={({ item }) => (
          <View className="flex-row items-center p-4 border-b border-gray-700">
            <Text className="w-1/6 text-white">{item.name}</Text>
            <Text className="w-1/6 text-gray-300">{item.category}</Text>
            <Text className="w-1/6 text-white">{item.stockQuantity}</Text>
            <Text className="w-1/6 text-gray-300">{item.unit}</Text>
            <Text className="w-1/6 text-gray-300">${item.cost.toFixed(2)}</Text>
            <Text className="w-1/6 text-white font-semibold">
...
// --- MODIFIED: Corrected LowStockReport Component ---
const LowStockReport = () => {
  ...
  return (
    <View className="flex-1 bg-[#303030] border border-gray-700 rounded-xl">
      <View className="flex-row p-4 bg-gray-800/50 rounded-t-xl border-b border-gray-700">
        {headers.map((h) => (
          <Text key={h} className="w-1/5 font-bold text-sm text-gray-400">
            {h}
          </Text>
        ))}
      </View>
      <FlatList
        ...
        renderItem={({ item }) => (
          <View className="flex-row items-center p-4 border-b border-gray-700">
            <Text className="w-1/5 text-white">{item.name}</Text>
            <Text className="w-1/5 text-gray-300">{item.category}</Text>
            <Text className="w-1/5 text-red-400 font-bold">
              {item.stockQuantity}
            </Text>
            <Text className="w-1/5 text-gray-300">{item.reorderThreshold}</Text>
            <Text className="w-1/5 text-gray-300">{item.vendorId}</Text>
          </View>
        )}
        ListEmptyComponent={
...
const PlaceholderReport = ({ title }: { title: string }) => (
  <View className="flex-1 justify-center items-center bg-[#303030] border border-gray-700 rounded-xl">
    <Text className="text-2xl text-gray-500">{title}</Text>
          ...
          {REPORT_TABS.map((tab) => {
            ...
            return (
              <TouchableOpacity
                ...
                className={`py-2 px-4 rounded-lg ${isActive ? "bg-blue-600" : "bg-[#303030] border border-gray-700"}`}
              >
                <Text
