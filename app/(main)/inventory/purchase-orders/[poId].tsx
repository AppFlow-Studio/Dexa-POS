...
import { Calendar, Camera, CreditCard, DollarSign, X } from "lucide-react-native";
...
const PurchaseOrderDetailScreen = () => {
  ...
  const { purchaseOrders, vendors, inventoryItems, logDeliveryForPO, logPaymentForPO, cancelPurchaseOrder, submitPurchaseOrder, deletePurchaseOrder } =
    useInventoryStore();
  ...
  const adjustReceivedQty = (inventoryItemId: string, qty: number) => {
    setReceivedDraft((prev) => ({ ...prev, [inventoryItemId]: Math.max(0, qty) }));
  };
      ...
      <View className="p-6">
        ...
        <View className="bg-[#303030] border border-gray-700 rounded-xl p-6 mb-6">
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-2xl font-bold text-white">PO Details: {po.poNumber}</Text>
            <View className={`px-3 py-1 rounded-full ${statusColors[po.status]}`}>
        ...
        {po.status === "Draft" && (
          <View className="bg-[#303030] border border-gray-700 rounded-xl p-6 mb-6">
            <Text className="text-xl font-bold text-white mb-3">Draft Actions</Text>
            <View className="flex-row gap-3">
              <TouchableOpacity
                onPress={handleSubmitDraft}
                className="flex-1 bg-blue-600 rounded-lg p-4 items-center"
              >
                <Text className="text-white font-bold">Submit to Pending Delivery</Text>
              </TouchableOpacity>
              <TouchableOpacity
        ...
        {po.status === "Pending Delivery" && (
          <View className="bg-[#303030] border border-gray-700 rounded-xl p-6 mb-6">
            <View className="flex-row items-center mb-4 gap-x-2">
              ...
              <Text className="text-xl font-bold text-white">Log Delivery & Goods Receipt</Text>
            </View>
            ...
            {!showDeliveryForm ? (
              ...
              >
                <Text className="text-white font-bold text-lg">Log Goods Received</Text>
              </TouchableOpacity>
            ) : (
              <View>
                <View className="mb-4">
                  <Text className="text-white font-semibold mb-2">Date & Time of Delivery</Text>
                  <TextInput
                    value={new Date(deliveryDate).toLocaleString()}
                    editable={false}
                    className="bg-[#212121] border border-gray-600 rounded-lg p-3 text-white"
                  />
                </View>

                {/* Editable Received Items */}
                <View className="mb-4">
                  <Text className="text-white font-semibold mb-2">Received Items (edit quantities or remove)</Text>
                  {po.items.map((li) => {
                    const invItem = inventoryItems.find((i) => i.id === li.inventoryItemId);
                    return (
                      <View key={li.inventoryItemId} className="flex-row items-center gap-2 py-2 border-b border-gray-700">
                        <Text className="flex-1 text-white">{invItem?.name}</Text>
                        <TouchableOpacity
                          onPress={() => adjustReceivedQty(li.inventoryItemId, Math.max(0, (receivedDraft[li.inventoryItemId] ?? li.quantity) - 1))}
                          className="w-8 h-8 rounded-lg bg-[#212121] border border-gray-600 items-center justify-center"
                        >
                          <Text className="text-white">-</Text>
                        </TouchableOpacity>
                        <TextInput
                          value={String(receivedDraft[li.inventoryItemId] ?? li.quantity)}
                          onChangeText={(t) => adjustReceivedQty(li.inventoryItemId, Number(t) || 0)}
                          keyboardType="numeric"
                          className="w-16 text-center bg-[#212121] border border-gray-600 rounded-lg p-2 text-white"
                        />
                        <TouchableOpacity
                          onPress={() => adjustReceivedQty(li.inventoryItemId, (receivedDraft[li.inventoryItemId] ?? li.quantity) + 1)}
                          className="w-8 h-8 rounded-lg bg-[#212121] border border-gray-600 items-center justify-center"
                        >
                          <Text className="text-white">+</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                          onPress={() => adjustReceivedQty(li.inventoryItemId, 0)}
                ...
                <View className="mb-4">
                  <Text className="text-white font-semibold mb-2">Upload Photos of Condition</Text>
                  <TouchableOpacity
                ...
                <View className="flex-row gap-3">
                  ...
                  >
                    <Text className="text-white font-semibold">Submit Delivery Log</Text>
                  </TouchableOpacity>
                </View>
              </View>
            )}
        ...
        {po.status === "Awaiting Payment" && (
          <View className="bg-[#303030] border border-gray-700 rounded-xl p-6 mb-6">
            ...
            {!showPaymentForm ? (
              ...
              >
                <Text className="text-white font-bold text-lg">Log Payment</Text>
              </TouchableOpacity>
            ) : (
              <View>
                <View className="mb-4">
                  ...
                  <View className="flex-row gap-3">
                    <TouchableOpacity
                      ...
                      className={`flex-1 p-3 rounded-lg border ${paymentMethod === "Card" ? "border-blue-500 bg-blue-500/20" : "border-gray-600"
                        }`}
                    >
                      <View className="flex-row items-center justify-center">
                        <CreditCard className="mr-2" size={20} color={paymentMethod === "Card" ? "#3B82F6" : "#9CA3AF"} />
                        <Text className={`font-semibold ${paymentMethod === "Card" ? "text-blue-400" : "text-gray-400"}`}>
                          Card
                        </Text>
                      </View>
                    </TouchableOpacity>
                    <TouchableOpacity
                      ...
                      className={`flex-1 p-3 rounded-lg border ${paymentMethod === "Cash" ? "border-green-500 bg-green-500/20" : "border-gray-600"
                        }`}
                    >
                      <View className="flex-row items-center justify-center">
                        <DollarSign className="mr-2" size={20} color={paymentMethod === "Cash" ? "#10B981" : "#9CA3AF"} />
                        <Text className={`font-semibold ${paymentMethod === "Cash" ? "text-green-400" : "text-gray-400"}`}>
                          Cash
                        </Text>
                      </View>
                    </TouchableOpacity>
                ...
                {paymentMethod === "Card" && (
                  <View className="mb-4">
                    <Text className="text-white font-semibold mb-2">Card Last 4 Digits</Text>
                    <TextInput
                      value={cardLast4}
                ...
                <View className="mb-4">
                  ...
                  <TextInput
                    value={vendor?.name || ""}
                    editable={false}
                    className="bg-[#404040] border border-gray-600 rounded-lg p-3 text-gray-300"
                  />
                </View>

                <View className="mb-4">
                  <Text className="text-white font-semibold mb-2">Paid To Employee</Text>
                  <TextInput
          ...
          <View className="bg-[#303030] border border-gray-700 rounded-xl p-6 mb-6">
            ...
            <View className="bg-[#212121] border border-gray-600 rounded-lg p-4">
              <View className="flex-row justify-between mb-2">
                ...
                <Text className="text-white">{new Date(po.deliveryLoggedAt).toLocaleString()}</Text>
              </View>
              {po.discrepancyNotes && (
                <View className="mb-2">
              ...
              {po.deliveryPhotos && po.deliveryPhotos.length > 0 && (
                <View>
                  <Text className="text-white font-semibold mb-2">Photos ({po.deliveryPhotos.length})</Text>
                  <View className="flex-row flex-wrap gap-2">
        ...
        {/* Discrepancy Report */}
        {(() => {
          const requested = (po.originalItems && po.originalItems.length > 0) ? po.originalItems : po.items;
          ...
          return (
            <View className="bg-[#303030] border border-gray-700 rounded-xl p-6 mb-6">
              <Text className="text-xl font-bold text-white mb-3">Discrepancy Report</Text>
              <View className="bg-[#212121] border border-gray-600 rounded-lg">
                <View className="flex-row px-4 py-3 border-b border-gray-700">
                  ...
                  <Text className="text-gray-400 w-20 text-right">Requested</Text>
                  <Text className="text-gray-400 w-20 text-right">Received</Text>
                ...
                {rows.map((r) => {
                  ...
                  return (
                    <View key={r.id} className="flex-row items-center px-4 py-3 border-b border-gray-800">
                      <Text className="text-white flex-1">{invItem?.name || r.id}</Text>
                      <Text className="text-gray-300 w-20 text-right">{r.reqQty}</Text>
                      <Text className="text-gray-300 w-20 text-right">{r.recQty}</Text>
                      <View className="w-24 items-end">
                        <Text className={`px-2 py-1 rounded text-xs font-semibold ${badge}`}>{status}</Text>
                      </View>
                    </View>
                  );
                })}
        ...
        {po.payment && (
          <View className="bg-[#303030] border border-gray-700 rounded-xl p-6 mb-6">
            <Text className="text-xl font-bold text-white mb-3">Payment History</Text>
            <View className="bg-[#212121] border border-gray-600 rounded-lg p-4">
              <View className="flex-row justify-between mb-2">
              ...
              <View className="flex-row justify-between mb-2">
                ...
                <Text className="text-white">${po.payment.amount.toFixed(2)}</Text>
              </View>
              ...
              {po.payment.paidToEmployee && (
                <View className="flex-row justify-between mb-2">
                  <Text className="text-white font-semibold">Paid To Employee</Text>
                  <Text className="text-white">{po.payment.paidToEmployee}</Text>
                </View>
              )}
              <View className="flex-row justify-between">
                <Text className="text-white font-semibold">Paid At</Text>
                <Text className="text-white">{new Date(po.payment.paidAt).toLocaleString()}</Text>
              </View>
            </View>
          </View>
        )}
        ...
        {po.status !== "Cancelled" && po.status !== "Paid" && (
          <View className="bg-[#303030] border border-gray-700 rounded-xl p-6 w-fit">
            ...
            >
              <Text className="text-white font-bold text-lg">Cancel Purchase Order</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {showCancelDialog && (
          <View className="absolute inset-0 bg-black/50 items-center justify-center">
            <View className="bg-[#303030] border border-gray-700 rounded-xl p-6 mx-6 w-11/12">
              <Text className="text-xl font-bold text-white mb-4">Cancel Purchase Order</Text>
              <Text className="text-gray-300 mb-6">
                Are you sure you want to cancel PO #{po.poNumber}? This action cannot be undone.
              </Text>
              <View className="flex-row gap-3">
                <TouchableOpacity
        ...
        {showDeleteDraftDialog && (
          <View className="absolute inset-0 bg-black/50 items-center justify-center">
            <View className="bg-[#303030] border border-gray-700 rounded-xl p-6 mx-6 w-11/12">
              <Text className="text-xl font-bold text-white mb-4">Delete Draft</Text>
              <Text className="text-gray-300 mb-6">
                Are you sure you want to delete draft {po.poNumber}? This action cannot be undone.
              </Text>
              <View className="flex-row gap-3">
                <TouchableOpacity
