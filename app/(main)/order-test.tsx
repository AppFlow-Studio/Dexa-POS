...
import { ChevronDown, ChevronUp, Copy, Minus, Plus, RotateCcw, Trash2 } from "lucide-react-native";
...
const OrderTest = () => {
    ...
    const defaultCategoryName = useMemo(() => {
        return menuItemsFromSync.length > 0 ? menuItemsFromSync[0].category_name : "";
    }, [menuItemsFromSync]);
    ...
    const [expandedModifierGroups, setExpandedModifierGroups] = useState<Record<string, boolean>>({});
    ...
    const handleModifierToggle = (groupId: string, itemId: string) => {
        const group = selectedMenuItem?.modifier_groups.find((g) => g.id === groupId);
        if (!group) return;
        ...
        setModifierSelections((prev) => {
            ...
            if (group.max_selections === 1) {
                ...
                if (!isCurrentlySelected && currentSelectedCount >= group.max_selections) {
    ...
    const handleModifierQuantityChange = (groupId: string, itemId: string, delta: number) => {
        setModifierSelections((prev) => {
            const newSelections = { ...prev };
            if (!newSelections[groupId]) {
                newSelections[groupId] = {};
    ...
    const validateModifierSelections = (): { valid: boolean; error?: string } => {
        ...
        for (const group of selectedMenuItem.modifier_groups) {
            ...
            const selectedCount = Object.values(selections).filter((sel) => sel.selected).length;
    ...
    const handleAddItem = async () => {
        ...
        try {
            ...
            const params: AddOrderItemParams = {
                ...
                p_modifiers: convertedModifiers.length > 0 ? convertedModifiers : undefined,
            };
    ...
    const handleCompleteFlow = async () => {
        ...
        try {
            ...
            try {
                ...
                const errorMessage = error?.message || "Failed to create order";
                addLog("Create Order (Complete Flow)", undefined, undefined, errorMessage);
            ...
            if ((menuItemId || locationExclusiveItemId) && selectedMenuItem) {
                try {
                    ...
                    const errorMessage = error?.message || "Failed to add item";
                    addLog("Add Item (Complete Flow)", undefined, undefined, errorMessage);
            ...
            try {
                ...
                const errorMessage = error?.message || "Failed to calculate tax";
                addLog("Calculate Tax (Complete Flow)", undefined, undefined, errorMessage);
            ...
            try {
                ...
                const errorMessage = error?.message || "Failed to update status";
                addLog("Update Status (Complete Flow)", undefined, undefined, errorMessage);
            ...
            if (amount) {
                try {
                    ...
                    addLog("Process Payment (Complete Flow)", paymentParams, paymentResult);
                } catch (error: any) {
                    const errorMessage = error?.message || "Failed to process payment";
                    addLog("Process Payment (Complete Flow)", undefined, undefined, errorMessage);
                        ...
                        <AccordionContent>
                            <View className="space-y-4">
                                ...
                                <View>
                                    ...
                                    <Select
                                        value={{ value: orderType, label: orderType.charAt(0).toUpperCase() + orderType.slice(1).replace('_', ' ') }}
                                        onValueChange={(option) => setOrderType(option?.value as OrderType)}
                                    >
                                        <SelectTrigger className="bg-[#303030] border-gray-600">
                                            <SelectValue placeholder="Select order type" className="text-white" />
                                        </SelectTrigger>
                            ...
                            <View className="space-y-4">
                                <View>
                                    ...
                                    {isLoadingMenu ? (
                                        <View className="bg-[#303030] border border-gray-600 rounded-lg px-3 py-2">
                                            <Text className="text-gray-400">Loading menu items...</Text>
                                        </View>
                                    ) : menuItemsFromSync.length > 0 ? (
                                        <Select
                                            value={selectedMenuItem ? { value: selectedMenuItem.id, label: `${selectedMenuItem.name} - $${selectedMenuItem.effective_price.toFixed(2)}` } : undefined}
                                            onValueChange={(option) => {
                                                const itemWithCategory = menuItemsFromSync.find((mi) => mi.menu_item.id === option?.value);
                                                if (itemWithCategory) {
                                                    setSelectedMenuItem(itemWithCategory.menu_item);
                                                    setSelectedCategoryName(itemWithCategory.category_name);
                                                } else {
                                        ...
                                        >
                                            <SelectTrigger className="bg-[#303030] border-gray-600">
                                                <SelectValue placeholder="Select a menu item" className="text-white" />
                                            </SelectTrigger>
                                            <SelectContent className="bg-[#212121] border-gray-600">
                                    ...
                                    <View className="flex-row items-center gap-3 p-3 bg-[#1a1a1a] rounded-lg border border-gray-700">
                                        ...
                                        <View className="flex-1">
                                            ...
                                            <Text className="text-gray-400 text-xs">
                                                Card: ${selectedMenuItem.effective_price.toFixed(2)} | Cash: ${selectedMenuItem.effective_cash_price.toFixed(2)}
                                            </Text>
                                        </View>
                                    </View>
                                )}
                                ...
                                {selectedMenuItem && selectedMenuItem.modifier_groups.length > 0 && (
                                    <View>
                                        ...
                                        {selectedMenuItem.modifier_groups.map((group) => {
                                            const isExpanded = expandedModifierGroups[group.id] ?? true;
                                            const selections = modifierSelections[group.id] || {};
                                            const selectedCount = Object.values(selections).filter(
                                                (sel) => sel.selected
                                            ).length;
                                            const isValidSelection =
                                                (!group.is_required || selectedCount >= group.min_selections) &&
                                                selectedCount <= group.max_selections &&
                                                (selectedCount === 0 || selectedCount >= group.min_selections);
                                                    ...
                                                    {isExpanded && (
                                                        <View className="px-3 pb-3 border-t border-gray-700">
                                                            {group.items.map((modifierItem) => {
                                                                const isSelected =
                                                                    selections[modifierItem.id]?.selected || false;
                                                                ...
                                                                return (
                                                                    <View
                                                                        key={modifierItem.id}
                                                                        className={`flex-row items-center justify-between py-2 border-b border-gray-800 last:border-b-0 ${isSelected
                                                                            ? "bg-blue-900/10"
                                                                            : ""
                                                                            }`}
                                                                    >
                                ...
                                {selectedMenuItem && (
                                    <View className="bg-blue-900/20 border border-blue-600 rounded-lg p-3">
                                        ...
                                        <Text className="text-white text-sm">
                                            Price Used: {useCashPrice && selectedMenuItem.effective_cash_price ? "Cash" : "Card"}
                                        </Text>
                                        <Text className="text-white text-sm">
                                            Available: {selectedMenuItem.effective_availability ? "Yes" : "No"}
                                        </Text>
                        ...
                        <AccordionContent>
                            <View className="space-y-4">
                                <View>
                                    ...
                                    <Select
                                        value={{ value: paymentMethod, label: paymentMethod.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()) }}
                                        onValueChange={(option) => setPaymentMethod(option?.value as PaymentMethod)}
                                    >
                                        <SelectTrigger className="bg-[#303030] border-gray-600">
                                        ...
                                        <SelectContent className="bg-[#212121] border-gray-600">
                                            ...
                                            <SelectItem value="card_dvpaylite" label="Card (DVPayLite)" />
                                ...
                                <View>
                                    ...
                                    <Select
                                        value={{ value: terminalType, label: terminalType.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()) }}
                                        onValueChange={(option) => setTerminalType(option?.value as TerminalType)}
                                    >
                                        <SelectTrigger className="bg-[#303030] border-gray-600">
                                            <SelectValue placeholder="Select terminal type" className="text-white" />
                                        </SelectTrigger>
                                        <SelectContent className="bg-[#212121] border-gray-600">
                                            <SelectItem value="dejavoo_spinapi" label="Dejavoo SPINAPI" />
                    ...
                    <AccordionItem value="status-config">
                        ...
                        <AccordionContent>
                            <View>
                                ...
                                <Select
                                    value={{ value: orderStatus, label: orderStatus.charAt(0).toUpperCase() + orderStatus.slice(1) }}
                                    onValueChange={(option) => setOrderStatus(option?.value as OrderStatus)}
                                >
                                    <SelectTrigger className="bg-[#303030] border-gray-600">
